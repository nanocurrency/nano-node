 ...
 */
size_t manually_count_pruned_blocks (nano::store::ledger_store & store)
{
	size_t count = 0;
...
TEST (system, block_sequence)
{
	...
	auto wallet = root->wallets.get_wallets ().begin ()->second;
	...
	for (auto rep : reps)
	{
		...
		std::cerr << rep.pub.to_account () << ' ' << pr->wallets.get_wallets ().begin ()->second->exists (rep.pub) << pr->weight (rep.pub) << ' ' << '\n';
	}
