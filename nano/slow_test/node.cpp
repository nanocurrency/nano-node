...
TEST (node, mass_vote_by_hash)
{
	...
	for (auto i (0); i < 10000; ++i)
	{
		auto block (std::make_shared<nano::state_block> (nano::dev::genesis_key.pub, previous, nano::dev::genesis_key.pub, nano::dev::constants.genesis_amount - (i + 1) * nano::MBAN_ratio, key.pub, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (previous)));
		previous = block->hash ();
...
TEST (confirmation_height, long_chains)
{
	...
	nano::send_block send (latest, key1.pub, nano::dev::constants.genesis_amount - nano::MBAN_ratio + num_blocks + 1, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (latest));
	...
	auto previous_destination_chain_hash = open.hash ();
	{
		...
		{
			nano::send_block send (previous_genesis_chain_hash, key1.pub, nano::dev::constants.genesis_amount - nano::MBAN_ratio + i + 1, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (previous_genesis_chain_hash));
			ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, send).code);
	...
	nano::send_block send1 (previous_destination_chain_hash, nano::dev::genesis_key.pub, nano::MBAN_ratio - 2, key1.prv, key1.pub, *system.work.generate (previous_destination_chain_hash));
	auto receive1 (std::make_shared<nano::state_block> (nano::dev::genesis_key.pub, previous_genesis_chain_hash, nano::dev::genesis->account (), nano::dev::constants.genesis_amount - nano::MBAN_ratio + 1, send1.hash (), nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (previous_genesis_chain_hash)));
	...
	nano::keypair key2;
	nano::state_block send2 (nano::dev::genesis->account (), receive1->hash (), nano::dev::genesis->account (), nano::dev::constants.genesis_amount - nano::MBAN_ratio, key2.pub, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (receive1->hash ()));
...
TEST (confirmation_height, prioritize_frontiers_overwrite)
{
	...
	nano::keypair key;
	auto latest_genesis = node->latest (nano::dev::genesis_key.pub);
	nano::send_block send (latest_genesis, key.pub, nano::MBAN_ratio - 1, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (latest_genesis));
	...
	nano::send_block send1 (send.hash (), key.pub, nano::MBAN_ratio - 2, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (send.hash ()));
...
TEST (telemetry, many_nodes)
{
	...
	nano::keypair key;
	nano::state_block send (nano::dev::genesis_key.pub, nano::dev::genesis->hash (), nano::dev::genesis_key.pub, nano::dev::constants.genesis_amount - nano::BAN_ratio, key.pub, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (nano::dev::genesis->hash ()));
...
TEST (node, wallet_create_block_confirm_conflicts)
{
	for (int i = 0; i < 5; ++i)
	{
		...
		nano::keypair key1;
		{
			...
			for (auto i = num_blocks - 1; i > 0; --i)
			{
				nano::send_block send (latest, key1.pub, nano::dev::constants.genesis_amount - nano::MBAN_ratio + i + 1, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (latest));
				ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, send).code);
				latest = send.hash ();
			}
		}
