...
TEST (system, receive_while_synchronizing)
{
	std::vector<boost::thread> threads;
	{
		...
		nano::transport::channel_udp sink (*node1, system.nodes[0]->network.endpoint ());
...
TEST (store, vote_load)
{
	...
	for (auto i (0); i < 1000000; ++i)
	{
		...
		node.vote_processor.vote (vote, std::make_shared<nano::transport::channel_udp> (*system.nodes[0], system.nodes[0]->network.endpoint ()));
	}
}
