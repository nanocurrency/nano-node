...
nano::error nano::rocksdb_config::serialize_toml (nano::tomlconfig & toml) const
{
	...
	toml.put ("read_cache", read_cache, "Total amount of megabytes allocated to the read cache, shared by all tables. Valid range is 1 - 16384. Default is 256.\nCarefully monitor memory usage if non-default values are used\ntype:long");
...
nano::error nano::rocksdb_config::deserialize_toml (nano::tomlconfig & toml)
{
	...
	if (read_cache < 1 || read_cache > 16384)
	{
		toml.get_error ().set ("read_cache must be between 1 and 16384 MB");
	}
