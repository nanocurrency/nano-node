...
namespace
{
void compare_telemetry_data_impl (const celerix::telemetry_data & data_a, const celerix::telemetry_data & data_b, bool & result)
{
	...
	ASSERT_EQ (data_a.major_version, celerix::get_major_node_version ());
	ASSERT_EQ (data_a.minor_version, celerix::get_minor_node_version ());
	ASSERT_EQ (data_a.patch_version, celerix::get_patch_node_version ());
	ASSERT_EQ (data_a.pre_release_version, celerix::get_pre_release_node_version ());
	ASSERT_EQ (data_a.maker, static_cast<std::underlying_type_t<celerix::telemetry_maker>> (celerix::telemetry_maker::nf_node));
...
bool celerix::test::compare_telemetry_data (const celerix::telemetry_data & data_a, const celerix::telemetry_data & data_b)
{
	bool result = false;
...
namespace
{
void compare_telemetry_impl (const celerix::telemetry_data & data, celerix::node const & node, bool & result)
{
	...
	celerix::telemetry_data data_l = data;
	data_l.signature.clear ();
	data_l.sign (node.node_id);
	ASSERT_NE (data.signature, data_l.signature);

	ASSERT_TRUE (celerix::test::compare_telemetry_data (data, node.local_telemetry ()));

	result = true;
}
}

bool celerix::test::compare_telemetry (const celerix::telemetry_data & data, const celerix::node & node)
{
	bool result = false;
