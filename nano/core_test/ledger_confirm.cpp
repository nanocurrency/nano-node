...
TEST (ledger_confirm, multiple_accounts)
{
	...
	ASSERT_FALSE (node->ledger.rollback (transaction, node->ledger->head (transaction, key2.pub)));
	ASSERT_FALSE (node->ledger.rollback (transaction, node->ledger->head (transaction, key1.pub)));
	ASSERT_TRUE (node->ledger.rollback (transaction, node->ledger->head (transaction, key1.pub)));
	ASSERT_TRUE (node->ledger.rollback (transaction, node->ledger->head (transaction, key2.pub)));

	// Confirm the other latest can't be rolled back either
	ASSERT_TRUE (node->ledger.rollback (transaction, node->ledger->head (transaction, key3.pub)));
	ASSERT_TRUE (node->ledger.rollback (transaction, node->ledger->head (transaction, nano::dev::genesis_key.pub)));
...
TEST (ledger_confirm, send_receive_between_2_accounts)
{
	...
	ASSERT_EQ (7, node->ledger->get (transaction, nano::dev::genesis_key.pub).value ().block_count);
	...
	ASSERT_EQ (5, node->ledger->get (transaction, key1.pub).value ().block_count);
...
TEST (ledger_confirm, send_receive_self)
{
	...
	ASSERT_TRUE (node->ledger.confirmed ().exists (transaction, receive3->hash ()));
	ASSERT_EQ (8, node->ledger->get (transaction, nano::dev::genesis_key.pub).value ().block_count);
...
TEST (ledger_confirm, all_block_types)
{
	...
	ASSERT_LE (4, node->ledger->get (transaction, nano::dev::genesis_key.pub).value ().block_count);
	ASSERT_EQ (3, node->store.confirmation_height.get (transaction, nano::dev::genesis_key.pub).value ().height);
	ASSERT_EQ (send1->hash (), node->store.confirmation_height.get (transaction, nano::dev::genesis_key.pub).value ().frontier);

	ASSERT_LE (7, node->ledger->get (transaction, key1.pub).value ().block_count);
	...
	ASSERT_EQ (8, node->ledger->get (transaction, key2.pub).value ().block_count);
...
TEST (ledger_confirm, observers)
{
	...
	ASSERT_TRUE (node1->ledger.confirmed ().exists (transaction, send1->hash ()));
