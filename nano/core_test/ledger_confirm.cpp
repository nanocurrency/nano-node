...
TEST (ledger_confirm, single)
{
	...
	celerix::block_hash latest1 (node->latest (celerix::dev::genesis_key.pub));
	...
	ASSERT_EQ (1, node->store.confirmation_height.get (transaction, celerix::dev::genesis_key.pub).value ().height);
	ASSERT_EQ (celerix::dev::genesis->hash (), node->store.confirmation_height.get (transaction, celerix::dev::genesis_key.pub).value ().frontier);
	...
	ASSERT_EQ (2, node->ledger.confirmed.account_height (transaction, celerix::dev::genesis_key.pub));
	ASSERT_EQ (send1->hash (), node->ledger.confirmed.account_head (transaction, celerix::dev::genesis_key.pub));
	...
	ASSERT_EQ (1, node->stats.count (celerix::stat::type::confirmation_height, celerix::stat::detail::blocks_confirmed, celerix::stat::dir::in));
...
TEST (ledger_confirm, multiple_accounts)
{
	...
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, open2));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, open3));

	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, send4));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, send5));

	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, receive1));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, send6));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, receive2));
	...
	auto receive3 = builder
					...
					.build ();
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, receive3));
	auto confirmed = node->ledger.confirm (transaction, receive3->hash ());
	ASSERT_EQ (10, confirmed.size ());
	ASSERT_EQ (10, node->stats.count (celerix::stat::type::confirmation_height, celerix::stat::detail::blocks_confirmed, celerix::stat::dir::in));
	ASSERT_EQ (11, node->ledger.cemented_count ());

	ASSERT_TRUE (node->ledger.confirmed.block_exists (transaction, receive3->hash ()));
	ASSERT_EQ (4, node->ledger.any.account_get (transaction, celerix::dev::genesis_key.pub).value ().block_count);
	ASSERT_EQ (4, node->ledger.confirmed.account_height (transaction, celerix::dev::genesis_key.pub));
	ASSERT_EQ (send3->hash (), node->ledger.confirmed.account_head (transaction, celerix::dev::genesis_key.pub));
	...
	ASSERT_TRUE (node->ledger.rollback (transaction, node->ledger.any.account_head (transaction, celerix::dev::genesis_key.pub)));
...
TEST (ledger_confirm, send_receive_between_2_accounts)
{
	...
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, receive4));
	...
	ASSERT_EQ (7, node->ledger.any.account_get (transaction, celerix::dev::genesis_key.pub).value ().block_count);
	ASSERT_EQ (6, node->ledger.confirmed.account_height (transaction, celerix::dev::genesis_key.pub));
	ASSERT_EQ (send5->hash (), node->ledger.confirmed.account_head (transaction, celerix::dev::genesis_key.pub));
...
TEST (ledger_confirm, send_receive_self)
{
	...
	ASSERT_EQ (6, node->stats.count (celerix::stat::type::confirmation_height, celerix::stat::detail::blocks_confirmed, celerix::stat::dir::in));

	ASSERT_TRUE (node->ledger.confirmed.block_exists (transaction, receive3->hash ()));
	ASSERT_EQ (8, node->ledger.any.account_get (transaction, celerix::dev::genesis_key.pub).value ().block_count);
	ASSERT_EQ (7, node->ledger.confirmed.account_height (transaction, celerix::dev::genesis_key.pub));
	ASSERT_EQ (receive3->hash (), node->ledger.confirmed.account_head (transaction, celerix::dev::genesis_key.pub));
...
TEST (ledger_confirm, all_block_types)
{
	...
	auto state_receive3 = builder
						  ...
						  .previous (send1->hash ())
						  .representative (celerix::dev::genesis_key.pub)
						  .balance (celerix::dev::constants.genesis_amount - celerix::Kcelerix_ratio * 2 + 1)
	...
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, state_open));

	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, send2));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, state_receive));

	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, state_send));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, receive));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, change));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, state_change));

	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, epoch));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, epoch1));

	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, state_send1));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, state_receive2));

	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, state_send2));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, state_send3));

	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, state_send4));
	ASSERT_EQ (celerix::block_status::progress, node->ledger.process (transaction, state_receive3));

	auto confirmed = node->ledger.confirm (transaction, state_send2->hash ());
	ASSERT_EQ (15, confirmed.size ());
	ASSERT_EQ (15, node->stats.count (celerix::stat::type::confirmation_height, celerix::stat::detail::blocks_confirmed, celerix::stat::dir::in));
	...
	ASSERT_LE (4, node->ledger.any.account_get (transaction, celerix::dev::genesis_key.pub).value ().block_count);
	ASSERT_EQ (3, node->ledger.confirmed.account_height (transaction, celerix::dev::genesis_key.pub));
	ASSERT_EQ (send1->hash (), node->ledger.confirmed.account_head (transaction, celerix::dev::genesis_key.pub));
...
TEST (ledger_confirm, conflict_rollback_cemented)
{
	...
	auto const genesis_hash = celerix::dev::genesis->hash ();
				  ...
				  .build ();
	{
		...
		ASSERT_EQ (celerix::block_status::progress, node1->ledger.process (transaction, fork1a));
		node1->ledger.confirm (transaction, fork1a->hash ());
	}
	ASSERT_TRUE (celerix::test::confirmed (*node1, { fork1a }));
	...
	ASSERT_TIMELY_EQ (5s, 1, (count = node1->stats.count (celerix::stat::type::ledger, celerix::stat::detail::rollback_failed)));
	ASSERT_TRUE (celerix::test::confirmed (*node1, { fork1a->hash () })); // fork1a should still remain after the rollback failed event
}
...
TEST (ledger_confirm, observers)
{
	...
	ASSERT_EQ (1, node1->stats.count (celerix::stat::type::confirmation_height, celerix::stat::detail::blocks_confirmed, celerix::stat::dir::in));
...
TEST (ledger_confirm, pruned_source)
{
	...
	auto store = celerix::make_store (system.logger, path, celerix::dev::constants);
	...
	ASSERT_EQ (celerix::block_status::progress, ledger.process (transaction, open2));
