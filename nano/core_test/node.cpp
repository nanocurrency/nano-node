...
TEST (node, balance)
{
	...
	ASSERT_EQ (std::numeric_limits<nano::uint128_t>::max (), system.nodes[0]->ledger->balance (transaction, nano::dev::genesis_key.pub));
}
...
TEST (node, fork_keep)
{
	...
	ASSERT_EQ (*send1, *winner.second);
	ASSERT_EQ (nano::dev::constants.genesis_amount - 100, winner.first);
	ASSERT_TRUE (node1.ledger->exists (transaction0, send1->hash ()));
	ASSERT_TRUE (node2.ledger->exists (transaction1, send1->hash ()));
}
...
TEST (node, fork_bootstrap_flip)
{
	...
	ASSERT_EQ (nano::block_status::progress, node1.ledger.process (node1.store.tx_begin_write (), send1));
	ASSERT_EQ (nano::block_status::progress, node2.ledger.process (node2.store.tx_begin_write (), send2));
	ASSERT_TRUE (node2.ledger->exists (node2.store.tx_begin_read (), send2->hash ()));
	...
	while (again)
	{
		...
		again = !node2.ledger->exists (node2.store.tx_begin_read (), send1->hash ());
	}
}
...
TEST (node, fork_open_flip)
{
	...
	ASSERT_TRUE (node1.ledger->exists (transaction1, open1->hash ()));
	ASSERT_TRUE (node2.ledger->exists (transaction2, open1->hash ()));
	ASSERT_FALSE (node2.ledger->exists (transaction2, open2->hash ()));
}
...
TEST (node, coherent_observer)
{
	...
	node1.observers.blocks.add ([&node1] (nano::election_status const & status_a, std::vector<nano::vote_with_weight_info> const &, nano::account const &, nano::uint128_t const &, bool, bool) {
		ASSERT_TRUE (node1.ledger->exists (node1.store.tx_begin_read (), status_a.winner->hash ()));
	});
	system.wallet (0)->insert_adhoc (nano::dev::genesis_key.prv);
	nano::keypair key;
