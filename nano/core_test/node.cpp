...
TEST (node, fork_flip)
{
	...
	ASSERT_EQ (node1.store.block_count (transaction1), 1);
	ASSERT_EQ (node1.ledger.cache.block_count, 2);
	ASSERT_EQ (node2.store.block_count (transaction2), 1);
...
TEST (node, DISABLED_fork_pre_confirm)
{
	...
	auto block2 = builder.make_block ()
	              ...
	              .previous (node0.latest (nano::test_genesis_key.pub))
	              .representative (key3.pub)
	              .balance (node0.balance (nano::test_genesis_key.pub))
	              .link (0)
	              .sign (nano::test_genesis_key.prv, nano::test_genesis_key.pub)
	              .work (0)
	              .build_shared ();
...
TEST (node, broadcast_elected)
{
	...
	for (auto & type : types)
	{
		...
		auto fork0 = builder.send ()
		             .previous (node2->latest (nano::test_genesis_key.pub))
		             .destination (rep_small.pub)
		             .balance (0)
		             .sign (nano::test_genesis_key.prv, nano::test_genesis_key.pub)
		             .work (*node0->work_generate_blocking (node2->latest (nano::test_genesis_key.pub)))
		             .build_shared ();
		// A copy is necessary to avoid data races during ledger processing, which sets the sideband
...
TEST (node, epoch_conflict_confirm)
{
	...
	ASSERT_TIMELY (3s, node1->block_confirmed (open->hash ()));
	{
		...
		ASSERT_EQ (nano::process_result::progress, node0->block_processor.process_one (transaction, events, send, num_state_blocks_added).code);
		ASSERT_EQ (nano::process_result::progress, node0->block_processor.process_one (transaction, events, send2, num_state_blocks_added).code);
		ASSERT_EQ (nano::process_result::progress, node0->block_processor.process_one (transaction, events, open, num_state_blocks_added).code);
		ASSERT_EQ (3, num_state_blocks_added);
	}
...
TEST (node, block_processor_signatures)
{
	...
	ASSERT_EQ (node1.store.block_count (transaction), 5);
