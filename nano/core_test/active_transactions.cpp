...
TEST (active_transactions, allow_limited_overflow)
{
	...
	ASSERT_TIMELY_EQ (5s, node.active.size (), node.active.limit () + node.hinting.limit ());
	// And it stays that way without increasing
	ASSERT_ALWAYS (1s, node.active.size () == node.active.limit () + node.hinting.limit ());
}
...
TEST (active_transactions, allow_limited_overflow_adapt)
{
	...
	ASSERT_TIMELY_EQ (5s, node.active.size (), node.hinting.limit ());
	// And it stays that way without increasing
	ASSERT_ALWAYS (1s, node.active.size () == node.hinting.limit ());
