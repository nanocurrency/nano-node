...
TEST (election_scheduler, activate_one_timely)
{
	...
	system.nodes[0]->scheduler.priority.activate (nano::dev::genesis_key.pub);
...
TEST (election_scheduler, no_vacancy)
{
	...
	node.scheduler.priority.activate (nano::dev::genesis_key.pub);
	...
	node.scheduler.priority.activate (key.pub);
