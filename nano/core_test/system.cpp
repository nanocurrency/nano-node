...
TEST (system, system_genesis)
{
	...
	for (auto & i : system.nodes)
	{
		...
		ASSERT_EQ (nano::dev::constants.genesis_amount, i->ledger->balance (transaction, nano::dev::genesis_key.pub));
	}
}
...
TEST (system, DISABLED_generate_send_existing)
{
	...
	auto info1 = node1.ledger->get (node1.store.tx_begin_read (), nano::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	...
	auto info2 = node1.ledger->get (node1.store.tx_begin_read (), nano::dev::genesis_key.pub);
	ASSERT_TRUE (info2);
	...
	while (info2->block_count < info1->block_count + 2)
	{
		...
		info2 = node1.ledger->get (transaction, nano::dev::genesis_key.pub);
		ASSERT_TRUE (info2);
	}
