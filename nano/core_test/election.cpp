...
TEST (election, quorum_minimum_flip_fail)
{
	...
	ASSERT_FALSE (node1.block_confirmed (send1->hash ()));
}
...
TEST (election, DISABLED_quorum_minimum_update_weight_before_quorum_checks)
{
	...
	auto vote1 (std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_max, nano::vote::duration_max, send1));
	ASSERT_EQ (nano::vote_code::vote, node1.active.vote (vote1));
	auto vote2 (std::make_shared<nano::vote> (key1.pub, key1.prv, nano::vote::timestamp_max, nano::vote::duration_max, send1));
