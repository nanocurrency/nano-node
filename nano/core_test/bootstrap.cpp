...
TEST (bootstrap_processor, process_new)
{
	...
	auto node3 (std::make_shared<nano::node> (system.io_ctx, nano::unique_path (), system.alarm, nano::node_config{ nano::get_available_port () }, system.work));
...
TEST (bootstrap_processor, lazy_unclear_state_link)
{
	...
	ASSERT_EQ (nano::process_result::progress, node1->process (*receive).code);
	// Start lazy bootstrap with last block in chain known
	auto node2 = system.add_node (nano::node_config{ nano::get_available_port () }, node_flags);
...
TEST (bulk, genesis)
{
	...
	auto node1 = system.add_node (config, node_flags);
	system.wallet (0)->insert_adhoc (nano::dev_genesis_key.prv);
	auto node2 (std::make_shared<nano::node> (system.io_ctx, nano::unique_path (), system.alarm, nano::node_config{ nano::get_available_port () }, system.work));
