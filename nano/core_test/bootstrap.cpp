...
TEST (bootstrap_processor, DISABLED_lazy_unclear_state_link)
{
	...
	ASSERT_TIMELY (5s, nano::test::block_or_pruned_exists (*node2, { send1, send2, open, receive }));
...
TEST (bootstrap_processor, lazy_unclear_state_link_not_existing)
{
	...
	ASSERT_TIMELY (15s, nano::test::block_or_pruned_exists (*node2, { send1, open, send2 }));
...
TEST (bootstrap_processor, DISABLED_lazy_destinations)
{
	...
	// Check processed blocks
	ASSERT_TIMELY (10s, !node2->bootstrap_initiator.in_progress ());
	ASSERT_TRUE (nano::test::block_or_pruned_exists (*node2, { send1, send2, open, state_open }));
}
...
TEST (bootstrap_processor, lazy_pruning_missing_block)
{
	...
	ASSERT_TRUE (nano::test::block_or_pruned_exists (*node1, { send1, send2, open, state_open }));
	// Start lazy bootstrap with last block in sender chain
	config.peering_port = system.get_available_port ();
	...
	ASSERT_FALSE (nano::test::block_or_pruned_exists (*node2, { send1, send2, open, state_open }));
	{
	...
	ASSERT_TRUE (nano::test::block_or_pruned_exists (*node2, { send1, send2 }));
	ASSERT_FALSE (nano::test::block_or_pruned_exists (*node2, { open, state_open }));
	node2->stop ();
}
...
TEST (bulk, DISABLED_genesis_pruning)
{
	...
	ASSERT_TRUE (nano::test::block_or_pruned_exists (*node2, { send1, send2, send3 }));
