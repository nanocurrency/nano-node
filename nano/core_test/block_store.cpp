...
TEST (block_store, large_iteration)
{
	...
	for (auto i (store->accounts_last (transaction)), n (store->accounts_end ()); i != n; --i)
	{
		nano::account current (i->first);
...
TEST (block_store, online_weight)
{
	...
	ASSERT_FALSE (store->init_error ());
	{
		...
		ASSERT_EQ (store->online_weight_end (), store->online_weight_last (transaction));
	...
	}
	{
		...
		auto item_last (store->online_weight_last (transaction));
		...
		ASSERT_EQ (store->online_weight_begin (transaction), store->online_weight_last (transaction));
