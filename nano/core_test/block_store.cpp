...
TEST (block_store, frontier_retrieval)
{
	...
	nano::account_info info1 (0, 0, 0, 0, nano::epoch::epoch_0);
	auto transaction (store->tx_begin_write ());
...
TEST (block_store, one_account)
{
	...
	store->account_put (transaction, account, { hash, account, hash, 42, nano::epoch::epoch_0 });
	auto begin (store->latest_begin (transaction));
	auto end (store->latest_end ());
	ASSERT_NE (end, begin);
...
TEST (block_store, two_account)
{
	...
	store->account_put (transaction, account1, { hash1, account1, hash1, 42, nano::epoch::epoch_0 });
	store->confirmation_height_put (transaction, account2, 30);
	store->account_put (transaction, account2, { hash2, account2, hash2, 84, nano::epoch::epoch_0 });
	auto begin (store->latest_begin (transaction));
	auto end (store->latest_end ());
	ASSERT_NE (end, begin);
...
TEST (block_store, latest_find)
{
	...
	auto transaction (store->tx_begin_write ());
	store->confirmation_height_put (transaction, account1, 0);
	store->account_put (transaction, account1, { hash1, account1, hash1, 0, nano::epoch::epoch_0 });
	store->confirmation_height_put (transaction, account2, 0);
	store->account_put (transaction, account2, { hash2, account2, hash2, 0, nano::epoch::epoch_0 });
...
void modify_account_info_to_v13 (nano::mdb_store & store, nano::transaction const & transaction_a, nano::account const & account)
{
	...
	nano::account_info_v13 account_info_v13 (info.head, info.rep_block, info.open_block, info.balance, state.modified (), state.block_count (), info.epoch ());
...
void modify_account_info_to_v14 (nano::mdb_store & store, nano::transaction const & transaction_a, nano::account const & account, uint64_t confirmation_height)
{
	...
	ASSERT_FALSE (store.account_get (transaction_a, account, info));
	nano::account_state state (transaction_a, store, info);
	nano::account_info_v14 account_info_v14 (info.head, info.rep_block, info.open_block, info.balance, state.modified (), state.block_count (), confirmation_height, info.epoch ());
