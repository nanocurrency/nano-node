...
TEST (websocket, confirmation)
{
	...
	auto send_amount = node1->delta () + 1;
...
TEST (websocket, vote)
{
	...
	system.wallet (0)->insert_adhoc (nano::dev_genesis_key.prv);
	nano::block_hash previous (node1->latest (nano::dev_genesis_key.pub));
	auto send (std::make_shared<nano::state_block> (nano::dev_genesis_key.pub, previous, nano::dev_genesis_key.pub, nano::genesis_amount - (node1->delta () + 1), key.pub, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (previous)));
	node1->process_active (send);
