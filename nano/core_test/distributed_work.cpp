...
TEST (distributed_work, stopped)
{
	...
	ASSERT_TRUE (system.nodes[0]->distributed_work.make (nano::work_version::work_0, nano::block_hash (), {}, {}, nano::network_constants::publish_test_threshold));
}
...
TEST (distributed_work, no_peers)
{
	...
	ASSERT_FALSE (node->distributed_work.make (nano::work_version::work_0, hash, node->config.work_peers, callback, node->network_params.network.publish_threshold, nano::account ()));
...
TEST (distributed_work, no_peers_disabled)
{
	...
	ASSERT_TRUE (node.distributed_work.make (nano::work_version::work_0, nano::block_hash (), node.config.work_peers, {}, nano::network_constants::publish_test_threshold));
}
...
TEST (distributed_work, no_peers_cancel)
{
	...
	ASSERT_FALSE (node.distributed_work.make (nano::work_version::work_0, hash, node.config.work_peers, callback_to_cancel, nano::difficulty::from_multiplier (1e6, node.network_params.network.publish_threshold)));
	ASSERT_EQ (1, node.distributed_work.items.size ());
...
TEST (distributed_work, no_peers_multi)
{
	...
	{
		ASSERT_FALSE (node->distributed_work.make (nano::work_version::work_0, hash, node->config.work_peers, callback, nano::difficulty::from_multiplier (10, node->network_params.network.publish_threshold)));
	}
	...
	for (unsigned i{ 0 }; i < total; ++i)
	{
		...
		ASSERT_FALSE (node->distributed_work.make (nano::work_version::work_0, hash_i, node->config.work_peers, callback, node->network_params.network.publish_threshold));
	}
...
TEST (distributed_work, peer)
{
	...
	ASSERT_FALSE (node->distributed_work.make (nano::work_version::work_0, hash, peers, callback, node->network_params.network.publish_threshold, nano::account ()));
	system.deadline_set (5s);
	while (!done)
	{
		ASSERT_NO_ERROR (system.poll ());
...
TEST (distributed_work, peer_malicious)
{
	...
	ASSERT_FALSE (node->distributed_work.make (nano::work_version::work_0, hash, peers, nullptr, node->network_params.network.publish_threshold, nano::account ()));
	system.deadline_set (5s);
