...
TEST (ledger_priority, genesis_priority)
{
	...
	auto const [priority_balance, priority_timestamp] = ledger.block_priority (transaction, *celerix::dev::genesis);
	ASSERT_EQ (celerix::dev::constants.genesis_amount, priority_balance);
...
TEST (ledger_priority, legacy_blocks_priority)
{
	...
	ASSERT_EQ (celerix::dev::constants.genesis_amount, send_balance);
	ASSERT_EQ (celerix::dev::genesis->sideband ().timestamp, send_timestamp);
	...
	ASSERT_EQ (celerix::dev::constants.genesis_amount - 100, open_balance);
...
TEST (ledger_priority, send_priority)
{
	...
	ASSERT_EQ (celerix::dev::constants.genesis_amount, priority_balance);
	ASSERT_EQ (celerix::dev::genesis->sideband ().timestamp, priority_timestamp);
}
...
TEST (ledger_priority, sequential_blocks)
{
	...
	ASSERT_EQ (celerix::dev::constants.genesis_amount, priority_balance1);
	ASSERT_EQ (100, priority_balance2);
	ASSERT_EQ (100, priority_balance3); // Max of current (50) and previous (100)

	ASSERT_EQ (celerix::dev::genesis->sideband ().timestamp, timestamp1); // genesis account
...
TEST (ledger_priority, block_fork)
{
	...
	auto send2b = builder
				  ...
				  .previous (send1->hash ())
				  .representative (celerix::dev::genesis_key.pub)
				  .balance (celerix::dev::constants.genesis_amount - 150)
				  .link (key2.pub)
				  .sign (celerix::dev::genesis_key.prv, celerix::dev::genesis_key.pub)
				  .work (*pool.generate (send1->hash ()))
				  .build ();
	...
	// Process first fork
	ASSERT_EQ (celerix::block_status::progress, ledger.process (transaction, send2a));

	// Try to process second fork
	ASSERT_EQ (celerix::block_status::fork, ledger.process (transaction, send2b));
	...
	// Process second fork
	ASSERT_EQ (celerix::block_status::progress, ledger.process (transaction, send2b));
	...
	ASSERT_EQ (celerix::dev::constants.genesis_amount - 100, priority_a);
	ASSERT_EQ (celerix::dev::constants.genesis_amount - 100, priority_b);
