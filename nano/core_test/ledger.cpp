...
TEST (votes, check_signature)
{
	...
	ASSERT_EQ (nano::vote_code::invalid, node1.vote_processor.vote_blocking (transaction, vote1, nano::message_sink_udp (node1, nano::endpoint (boost::asio::ip::address_v6 (), 0))));
	vote1->signature.bytes[0] ^= 1;
	ASSERT_EQ (nano::vote_code::vote, node1.vote_processor.vote_blocking (transaction, vote1, nano::message_sink_udp (node1, nano::endpoint (boost::asio::ip::address_v6 (), 0))));
	ASSERT_EQ (nano::vote_code::replay, node1.vote_processor.vote_blocking (transaction, vote1, nano::message_sink_udp (node1, nano::endpoint (boost::asio::ip::address_v6 (), 0))));
}
...
TEST (votes, add_old)
{
	...
	std::unique_lock<std::mutex> lock (node1.active.mutex);
	node1.vote_processor.vote_blocking (transaction, vote1, nano::message_sink_udp (node1, node1.network.endpoint ()));
	lock.unlock ();
	nano::keypair key2;
	auto send2 (std::make_shared<nano::send_block> (genesis.hash (), key2.pub, 0, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0));
	...
	votes1->last_votes[nano::test_genesis_key.pub].time = std::chrono::steady_clock::now () - std::chrono::seconds (20);
	lock.lock ();
	node1.vote_processor.vote_blocking (transaction, vote2, nano::message_sink_udp (node1, node1.network.endpoint ()));
	lock.unlock ();
	ASSERT_EQ (2, votes1->last_votes.size ());
	ASSERT_NE (votes1->last_votes.end (), votes1->last_votes.find (nano::test_genesis_key.pub));
	ASSERT_EQ (send1->hash (), votes1->last_votes[nano::test_genesis_key.pub].hash);
...
TEST (votes, add_old_different_account)
{
	...
	auto vote1 (std::make_shared<nano::vote> (nano::test_genesis_key.pub, nano::test_genesis_key.prv, 2, send1));
	std::unique_lock<std::mutex> lock (node1.active.mutex);
	auto vote_result1 (node1.vote_processor.vote_blocking (transaction, vote1, nano::message_sink_udp (node1, node1.network.endpoint ())));
	lock.unlock ();
	...
	lock.lock ();
	auto vote2 (std::make_shared<nano::vote> (nano::test_genesis_key.pub, nano::test_genesis_key.prv, 1, send2));
	auto vote_result2 (node1.vote_processor.vote_blocking (transaction, vote2, nano::message_sink_udp (node1, node1.network.endpoint ())));
	lock.unlock ();
