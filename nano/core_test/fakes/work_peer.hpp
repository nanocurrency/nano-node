...
public:
	fake_work_peer_connection (asio::io_context & ioc_a, fake_work_peer_type const type_a, celerix::work_version const version_a, celerix::work_pool & pool_a, std::function<void (bool const)> on_generation_a, std::function<void ()> on_cancel_a) :
		socket (ioc_a),
		type (type_a),
		version (version_a),
		work_pool (pool_a),
...
private:
	...
	void handle_generate (celerix::block_hash const & hash_a)
	{
		if (type == fake_work_peer_type::good)
		{
			...
			work_pool.generate (version, hash, request_difficulty, [this_l = shared_from_this (), hash] (boost::optional<uint64_t> work_a) {
				...
				ptree::ptree message_l;
				message_l.put ("work", celerix::to_string_hex (result));
				message_l.put ("difficulty", celerix::to_string_hex (result_difficulty));
				message_l.put ("multiplier", celerix::to_string (celerix::difficulty::to_multiplier (result_difficulty, this_l->work_pool.network_constants.work.threshold_base (this_l->version))));
...
public:
	...
	fake_work_peer (celerix::work_pool & pool_a, asio::io_context & ioc_a, unsigned short port_a, fake_work_peer_type const type_a, celerix::work_version const version_a = celerix::work_version::work_1) :
		pool (pool_a),
		ioc (ioc_a),
