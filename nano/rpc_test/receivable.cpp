...
TEST (rpc, receivable)
{
	...
	auto chain = celerix::test::setup_chain (system, *node, 1);
	auto block1 = chain[0];
	...
	celerix::block_hash hash{ blocks_node.begin ()->second.get<std::string> ("") };
...
TEST (rpc, receivable_sorting)
{
	...
	auto & blocks_node = response.get_child ("blocks");
	ASSERT_EQ (1, blocks_node.size ());
	celerix::block_hash hash{ blocks_node.begin ()->first };
	ASSERT_EQ (block1->hash (), hash);
...
TEST (rpc, receivable_unconfirmed)
{
	...
	celerix::test::confirm (node->ledger, block1);
	request.put ("include_only_confirmed", "true");
	ASSERT_TRUE (check_block_response_count (system, rpc_ctx, request, 1));
}
...
/*TEST (rpc, amounts)
{
	auto block2 (system.wallet (0)->send_action (celerix::dev::genesis_key.pub, key1.pub, 200));
	auto block3 (system.wallet (0)->send_action (celerix::dev::genesis_key.pub, key1.pub, 300));
	auto block4 (system.wallet (0)->send_action (celerix::dev::genesis_key.pub, key1.pub, 400));
	...
	request.put ("count", "2");
	{
		...
		celerix::block_hash hash (blocks_node.begin ()->first);
		celerix::block_hash hash1 ((++blocks_node.begin ())->first);
...
TEST (rpc, receivable_offset_and_sorting)
{
	...
	auto block1 = system.wallet (0)->send_action (celerix::dev::genesis_key.pub, key1.pub, 200);
	auto block2 = system.wallet (0)->send_action (celerix::dev::genesis_key.pub, key1.pub, 100);
	auto block3 = system.wallet (0)->send_action (celerix::dev::genesis_key.pub, key1.pub, 400);
	auto block4 = system.wallet (0)->send_action (celerix::dev::genesis_key.pub, key1.pub, 300);
	auto block5 = system.wallet (0)->send_action (celerix::dev::genesis_key.pub, key1.pub, 300);
	auto block6 = system.wallet (0)->send_action (celerix::dev::genesis_key.pub, key1.pub, 300);
	...
	ASSERT_FALSE (node->store.confirmation_height.get (node->store.tx_begin_read (), celerix::dev::genesis_key.pub, confirmation_height_info));
	...
	auto hash_exists = [] (boost::property_tree::ptree & node, bool match_first, celerix::block_hash hash) {
		std::stringstream ss;
	...
	request.put ("offset", "0");
	{
		...
		ASSERT_EQ (block3->hash (), celerix::block_hash{ itr->first });
		...
		++itr;
		ASSERT_EQ (block1->hash (), celerix::block_hash{ itr->first });
	...
	request.put ("offset", "3");
	{
		...
		++itr;
		ASSERT_EQ (block2->hash (), celerix::block_hash{ itr->first });
...
TEST (rpc, receivable_burn)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (celerix::dev::genesis_key.prv);
	auto block1 (system.wallet (0)->send_action (celerix::dev::genesis_key.pub, celerix::dev::constants.burn_account, 100));
	auto const rpc_ctx = add_rpc (system, node);
	ASSERT_TIMELY (5s, node->block_confirmed (block1->hash ()));
	boost::property_tree::ptree request;
	request.put ("action", "receivable");
	request.put ("account", celerix::dev::constants.burn_account.to_account ());
...
TEST (rpc, accounts_pending_deprecated)
{
	...
	child.put ("", celerix::dev::genesis_key.pub.to_account ());
...
TEST (rpc, accounts_receivable_blocks)
{
	...
	for (auto & blocks : response.get_child ("blocks"))
	{
		...
		celerix::block_hash hash1{ blocks.second.begin ()->second.get<std::string> ("") };
...
TEST (rpc, accounts_receivable_sorting)
{
	...
	for (auto & blocks : response.get_child ("blocks"))
	{
		std::string account_text{ blocks.first };
		ASSERT_EQ (block1->destination ().to_account (), account_text);
		celerix::block_hash hash1{ blocks.second.begin ()->first };
		ASSERT_EQ (block1->hash (), hash1);
...
TEST (rpc, accounts_receivable_source)
{
	celerix::test::system system;
	auto node = add_ipc_enabled_node (system);
	auto chain = celerix::test::setup_chain (system, *node, 1, celerix::dev::genesis_key);
	auto block1 = chain[0];
