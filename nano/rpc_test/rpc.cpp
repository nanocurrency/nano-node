...
TEST (rpc, history)
{
	...
	nano::genesis genesis;
	nano::state_block usend (nano::genesis_account, node0->latest (nano::genesis_account), nano::genesis_account, nano::genesis_amount - nano::MBAN_ratio, nano::genesis_account, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *node0->work_generate_blocking (node0->latest (nano::genesis_account)));
...
TEST (rpc, history_pruning)
{
	...
	auto usend (std::make_shared<nano::state_block> (nano::genesis_account, receive->hash (), nano::genesis_account, nano::genesis_amount - nano::MBAN_ratio, nano::genesis_account, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *node0->work_generate_blocking (receive->hash ())));
...
TEST (rpc, process_subtype_send)
{
	...
	nano::keypair key;
	auto latest (node1.latest (nano::dev_genesis_key.pub));
	nano::state_block send (nano::genesis_account, latest, nano::genesis_account, nano::genesis_amount - nano::MBAN_ratio, key.pub, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	nano::node_rpc_config node_rpc_config;
	nano::ipc::ipc_server ipc_server (node1, node_rpc_config);
	nano::rpc_config rpc_config (nano::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
...
TEST (rpc, process_subtype_open)
{
	...
	nano::keypair key;
	auto latest (node1.latest (nano::dev_genesis_key.pub));
	auto send = std::make_shared<nano::state_block> (nano::genesis_account, latest, nano::genesis_account, nano::genesis_amount - nano::MBAN_ratio, key.pub, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
...
TEST (rpc, process_subtype_receive)
{
	...
	auto & node2 = *system.add_node ();
	auto latest (node1.latest (nano::dev_genesis_key.pub));
	auto send = std::make_shared<nano::state_block> (nano::genesis_account, latest, nano::genesis_account, nano::genesis_amount - nano::MBAN_ratio, nano::dev_genesis_key.pub, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	ASSERT_EQ (nano::process_result::progress, node1.process (*send).code);
	ASSERT_EQ (nano::process_result::progress, node2.process (*send).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	node1.scheduler.manual (send);
...
TEST (rpc, process_ledger_insufficient_work)
{
	...
	nano::state_block send (nano::genesis_account, latest, nano::genesis_account, nano::genesis_amount - nano::MBAN_ratio, nano::dev_genesis_key.pub, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, system.work_generate_limited (latest, min_difficulty, max_difficulty));
...
TEST (rpc, work_generate_block_high)
{
	...
	rpc.start ();
	nano::keypair key;
	nano::state_block block (key.pub, 0, nano::dev_genesis_key.pub, nano::MBAN_ratio, 123, key.prv, key.pub, *node->work_generate_blocking (key.pub));
...
TEST (rpc, work_generate_block_low)
{
	...
	rpc.start ();
	nano::keypair key;
	nano::state_block block (key.pub, 0, nano::dev_genesis_key.pub, nano::MBAN_ratio, 123, key.prv, key.pub, 0);
...
TEST (rpc, work_generate_block_ledger_epoch_2)
{
	...
	nano::keypair key;
	system.wallet (0)->insert_adhoc (nano::dev_genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (nano::dev_genesis_key.pub, key.pub, nano::MBAN_ratio));
	...
	nano::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	nano::state_block block (key.pub, 0, nano::dev_genesis_key.pub, nano::MBAN_ratio, send_block->hash (), key.prv, key.pub, 0);
...
TEST (rpc, block_count_pruning)
{
	...
	auto latest (node1.latest (nano::dev_genesis_key.pub));
	auto send1 (std::make_shared<nano::send_block> (latest, nano::dev_genesis_key.pub, nano::genesis_amount - nano::MBAN_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *node1.work_generate_blocking (latest)));
...
TEST (rpc, ban_to_raw)
{
	...
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (nano::BAN_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, banoshi_to_raw)
{
	...
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (nano::banoshi_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, nano_to_raw)
{
	...
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (nano::RAW_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, nano_from_raw)
{
	...
	request1.put ("amount", nano::RAW_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ ("1", response1.json.get<std::string> ("amount"));
...
TEST (rpc, json_block_input)
{
	...
	nano::keypair key;
	system.wallet (0)->insert_adhoc (key.prv);
	nano::state_block send (nano::genesis_account, node1.latest (nano::dev_genesis_key.pub), nano::genesis_account, nano::genesis_amount - nano::MBAN_ratio, key.pub, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, 0);
	scoped_io_thread_name_change scoped_thread_name_io;
	nano::node_rpc_config node_rpc_config;
	nano::ipc::ipc_server ipc_server (node1, node_rpc_config);
	nano::rpc_config rpc_config (nano::get_available_port (), true);
...
TEST (rpc, blocks_info_subtype)
{
	...
	system.wallet (0)->insert_adhoc (nano::dev_genesis_key.prv);
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (nano::dev_genesis_key.pub, nano::dev_genesis_key.pub, nano::MBAN_ratio));
	ASSERT_NE (nullptr, send);
...
TEST (rpc, block_create_state)
{
	...
	request.put ("balance", (nano::genesis_amount - nano::MBAN_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_receive_epoch_v2)
{
	...
	auto send_block (system.wallet (0)->send_action (nano::dev_genesis_key.pub, key.pub, nano::MBAN_ratio));
	ASSERT_NE (nullptr, send_block);
	nano::state_block open (key.pub, 0, nano::dev_genesis_key.pub, nano::MBAN_ratio, send_block->hash (), key.prv, key.pub, *node->work_generate_blocking (key.pub));
	ASSERT_EQ (nano::process_result::progress, node->process (open).code);
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (*node, nano::epoch::epoch_2));
	auto send_block_2 (system.wallet (0)->send_action (nano::dev_genesis_key.pub, key.pub, nano::MBAN_ratio));
	boost::property_tree::ptree request;
	request.put ("action", "block_create");
	request.put ("type", "state");
	request.put ("key", key.prv.to_string ());
...
TEST (rpc, online_reps)
{
	...
	ASSERT_EQ (node2->online_reps.online (), nano::genesis_amount - nano::MBAN_ratio);
	nano::node_rpc_config node_rpc_config;
...
TEST (rpc, confirmation_height_currently_processing)
{
	...
	auto previous_genesis_chain_hash = node->latest (nano::dev_genesis_key.pub);
	{
		auto transaction = node->store.tx_begin_write ();
		nano::keypair key1;
		nano::send_block send (previous_genesis_chain_hash, key1.pub, nano::genesis_amount - nano::MBAN_ratio - 1, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (previous_genesis_chain_hash));
...
TEST (rpc, confirmation_history)
{
	...
	auto block (system.wallet (0)->send_action (nano::dev_genesis_key.pub, key.pub, nano::MBAN_ratio));
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	debug_assert (tally_num == nano::genesis_amount || tally_num == (nano::genesis_amount - nano::MBAN_ratio));
	system.stop ();
}
...
TEST (rpc, confirmation_history_hash)
{
	...
	system.wallet (0)->insert_adhoc (nano::dev_genesis_key.prv);
	ASSERT_TRUE (node->active.list_recently_cemented ().empty ());
	auto send1 (system.wallet (0)->send_action (nano::dev_genesis_key.pub, key.pub, nano::MBAN_ratio));
	auto send2 (system.wallet (0)->send_action (nano::dev_genesis_key.pub, key.pub, nano::MBAN_ratio));
	auto send3 (system.wallet (0)->send_action (nano::dev_genesis_key.pub, key.pub, nano::MBAN_ratio));
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	nano::amount tally_num;
	tally_num.decode_dec (tally);
	debug_assert (tally_num == nano::genesis_amount || tally_num == (nano::genesis_amount - nano::MBAN_ratio) || tally_num == (nano::genesis_amount - 2 * nano::MBAN_ratio) || tally_num == (nano::genesis_amount - 3 * nano::MBAN_ratio));
	system.stop ();
}
...
TEST (rpc, account_lazy_start)
{
	...
	auto send1 (std::make_shared<nano::state_block> (nano::dev_genesis_key.pub, nano::genesis_hash, nano::dev_genesis_key.pub, nano::genesis_amount - nano::MBAN_ratio, key.pub, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (nano::genesis_hash)));
