...
void nano::block_check_context::load (store::transaction const & transaction)
{
	...
	if (ledger->exists_or_pruned (transaction, hash))
	{
	...
	{
		previous = ledger->get (transaction, block.previous ());
	}
	if (!gap_previous ())
	{
		auto account_l = account ();
		auto source_l = source ();
		state = ledger->get (transaction, account_l);
		if (!state)
		{
			state = nano::account_info{};
		}
		source_exists = ledger->exists_or_pruned (transaction, source_l);
		nano::pending_key key{ account_l, source_l };
		receivable = ledger->get (transaction, key);
		any_receivable = ledger->receivable_any (transaction, account_l);
