...
public:
	...
	void send_block (nano::send_block const & block_a) override
	{
		...
		if (!error)
		{
			...
			[[maybe_unused]] auto error (ledger.store.account.account_get (transaction, pending.source, info));
			debug_assert (!error);
	...
	void receive_block (nano::receive_block const & block_a) override
	{
		...
		nano::account_info info;
		[[maybe_unused]] auto error (ledger.store.account.account_get (transaction, destination_account, info));
		debug_assert (!error);
	...
	void change_block (nano::change_block const & block_a) override
	{
		...
		nano::account_info info;
		[[maybe_unused]] auto error (ledger.store.account.account_get (transaction, account, info));
		debug_assert (!error);
	...
	void state_block (nano::state_block const & block_a) override
	{
		...
		nano::account_info info;
		auto error (ledger.store.account.account_get (transaction, block_a.hashables.account, info));
	...
	if (result.code == nano::process_result::progress)
	{
		...
		if (result.code == nano::process_result::progress)
		{
			...
			if (result.code == nano::process_result::progress)
			{
				...
				auto account_error (ledger.store.account.account_get (transaction, block_a.hashables.account, info));
				if (!account_error)
				{
	...
	if (result.code == nano::process_result::progress)
	{
		...
		if (result.code == nano::process_result::progress)
		{
			...
			if (result.code == nano::process_result::progress)
			{
				...
				{
					nano::account_info info;
					auto latest_error (ledger.store.account.account_get (transaction, account, info));
					(void)latest_error;
			...
			if (result.code == nano::process_result::progress)
			{
				...
				if (result.code == nano::process_result::progress)
				{
					...
					if (result.code == nano::process_result::progress)
					{
						...
						{
							nano::account_info info;
							ledger.store.account.account_get (transaction, account, info);
...
void ledger_processor::open_block (nano::open_block & block_a)
{
	...
	if (result.code == nano::process_result::progress)
	{
		...
		if (result.code == nano::process_result::progress)
		{
			...
			{
				nano::account_info info;
				result.code = ledger.store.account.account_get (transaction, block_a.hashables.account, info) ? nano::process_result::progress : nano::process_result::fork; // Has this account already been opened? (Malicious)
				if (result.code == nano::process_result::progress)
				{
...
void nano::ledger::initialize (nano::generate_cache const & generate_cache_a)
{
	...
	{
		store.account.accounts_for_each_par (
...
nano::uint128_t nano::ledger::account_balance (nano::transaction const & transaction_a, nano::account const & account_a, bool only_confirmed_a)
{
	...
	{
		nano::account_info info;
		auto none (store.account.account_get (transaction_a, account_a, info));
		if (!none)
		{
...
bool nano::ledger::rollback (nano::write_transaction const & transaction_a, nano::block_hash const & block_a, std::vector<std::shared_ptr<nano::block>> & list_a)
{
	...
	while (!error && store.block_exists (transaction_a, block_a))
	{
		...
		{
			auto latest_error = store.account.account_get (transaction_a, account_l, account_info);
			debug_assert (!latest_error);
...
{
	nano::account_info info;
	auto latest_error (store.account.account_get (transaction_a, account_a, info));
...
{
	nano::account_info info;
	if (store.account.account_get (transaction_a, account_a, info))
	{
		return account_a;
	}
	else
...
void nano::ledger::update_account (nano::write_transaction const & transaction_a, nano::account const & account_a, nano::account_info const & old_a, nano::account_info const & new_a)
{
	if (!new_a.head.is_zero ())
	{
		...
		if (!old_a.head.is_zero () && old_a.epoch () != new_a.epoch ())
		{
			...
			store.account.account_del (transaction_a, account_a);
		}
		store.account.account_put (transaction_a, account_a, new_a);
	}
	else
	{
...
std::shared_ptr<nano::block> nano::ledger::successor (nano::transaction const & transaction_a, nano::qualified_root const & root_a)
{
	...
	{
		nano::account_info info;
		if (!store.account.account_get (transaction_a, root_a.root ().as_account (), info))
		{
...
std::shared_ptr<nano::block> nano::ledger::forked_block (nano::transaction const & transaction_a, nano::block const & block_a)
{
	...
	{
		nano::account_info info;
		auto error (store.account.account_get (transaction_a, root.as_account (), info));
		(void)error;
		debug_assert (!error);
...
std::multimap<uint64_t, nano::uncemented_info, std::greater<>> nano::ledger::unconfirmed_frontiers () const
{
	...
	store.account.accounts_for_each_par ([this, &result] (nano::read_transaction const & transaction_a, nano::store_iterator<nano::account, nano::account_info> i, nano::store_iterator<nano::account, nano::account_info> n) {
	...
	if (!rocksdb_store->init_error ())
	{
		...
		[&rocksdb_store] (nano::read_transaction const & /*unused*/, auto i, auto n) {
			for (; i != n; ++i)
			{
				...
				rocksdb_store->account.account_put (rocksdb_transaction, i->first, i->second);
			}
		});
		...
		nano::account_info account_info;
		error |= rocksdb_store->account.account_get (rocksdb_transaction, account, account_info);
