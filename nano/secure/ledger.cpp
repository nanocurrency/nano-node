		...
		if (result.code == nano::process_result::progress)
		{
			...
			if (result.code == nano::process_result::progress)
			{
				...
				else
				{
					...
					// Non-exisiting account should have pending entries
					if (result.code == nano::process_result::progress)
					{
	...
	return std::all_of (dependencies.begin (), dependencies.end (), [this, &transaction_a](nano::block_hash const & hash_a) {
		...
		if (!result)
		{
			...
			if (block != nullptr)
			{
				...
				auto error = store.confirmation_height_get (transaction_a, block->account (), height);
				debug_assert (!error);
				result = block->height () <= height.height;
			}
		}
		return result;
	});
