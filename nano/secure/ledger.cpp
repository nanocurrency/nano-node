...
bool nano::ledger::migrate_lmdb_to_rocksdb (boost::filesystem::path const & data_path_a) const
{
	...
	if (!rocksdb_store->init_error ())
	{
		...
		for (auto i (store.online_weight.online_weight_begin (lmdb_transaction)), n (store.online_weight.online_weight_end ()); i != n; ++i)
		{
			rocksdb_store->online_weight.online_weight_put (rocksdb_transaction, i->first, i->second);
		}
		...
		error |= store.online_weight.online_weight_count (lmdb_transaction) != rocksdb_store->online_weight.online_weight_count (rocksdb_transaction);
