...
wallet (wallet_a)
{
	...
	QObject::connect (copy_button, &QPushButton::clicked, [this]() {
		...
		this->wallet.node.workers.add_delayed_task (std::chrono::steady_clock::now () + std::chrono::seconds (2), [this]() {
	...
	QObject::connect (create_account, &QPushButton::released, [this]() {
		{
			...
			if (this->wallet.wallet_m->store.valid_password (transaction))
			{
				...
				this->wallet.node.workers.add_delayed_task (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
					this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
...
void nano_qt::accounts::refresh_wallet_balance ()
{
	...
	this->wallet.node.workers.add_delayed_task (std::chrono::steady_clock::now () + std::chrono::seconds (60), [this]() {
		this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
			refresh_wallet_balance ();
		}));
	});
			...
			if (!seed_l.data.decode_hex (seed->text ().toStdString ()))
			{
				bool successful (false);
				{
					...
					else
					{
						...
						this->wallet.node.workers.add_delayed_task (std::chrono::steady_clock::now () + std::chrono::seconds (10), [this]() {
							this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
								show_line_ok (*seed);
...
void nano_qt::block_viewer::rebroadcast_action (nano::block_hash const & hash_a)
{
	...
	if (block != nullptr)
	{
		...
		if (!successor.is_zero ())
		{
			...
			wallet.node.workers.add_delayed_task (std::chrono::steady_clock::now () + std::chrono::seconds (1), [this, successor]() {
...
void nano_qt::wallet::ongoing_refresh ()
{
	...
	node.workers.add_delayed_task (std::chrono::steady_clock::now () + std::chrono::seconds (5), [wallet_w]() {
				...
				if (!parse_error)
				{
					...
					if (actual <= balance)
					{
						...
						else
						{
							...
							this_l->node.workers.add_delayed_task (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this_w]() {
								if (auto this_l = this_w.lock ())
								{
...
{
	this->node.workers.add_delayed_task (std::chrono::steady_clock::now () + std::chrono::seconds (3), [this]() {
