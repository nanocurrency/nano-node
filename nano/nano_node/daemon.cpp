...
void nano_daemon::daemon::run (boost::filesystem::path const & data_path, nano::node_flags const & flags)
{
	...
	if (!error)
	{
		...
		nano::work_pool opencl_work (config.node.work_threads, config.node.work_pow_calc_interval, opencl ? [&opencl](nano::uint256_union const & root_a, uint64_t difficulty_a) {
