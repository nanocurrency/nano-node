...
void nano_daemon::daemon::run (boost::filesystem::path const & data_path, nano::node_flags const & flags)
{
	...
	if (!error)
	{
		...
		if (opencl != nullptr)
		{
			env.work_impl = std::make_unique<nano::work_pool> (config.node.work_threads, config.node.pow_sleep_interval, [&opencl](nano::work_version const version_a, nano::root const & root_a, uint64_t difficulty_a, std::atomic<int> & ticket_a) {
			...
			if (!node->init_error ())
			{
				...
				if (config.rpc_enable)
				{
					if (!config.rpc.child_process.enable)
					{
						...
						rpc_handler = std::make_unique<nano::inprocess_rpc_handler> (*node, ipc_server, config.rpc, [&ipc_server, &env]() {
							ipc_server.stop ();
							env.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (3), [&env]() {
								env.ctx.stop ();
							});
						});
				...
				runner = std::make_unique<nano::thread_runner> (env.ctx, node->config.io_threads);
				runner->join ();
