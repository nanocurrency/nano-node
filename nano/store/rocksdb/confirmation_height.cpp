...
{
	auto status = store.put (transaction, store.table_to_column_family (tables::confirmation_height), account, confirmation_height_info);
...
bool nano::store::rocksdb::confirmation_height::get (store::transaction const & transaction, nano::account const & account, nano::confirmation_height_info & confirmation_height_info)
{
	...
	auto status = store.get (transaction, store.table_to_column_family (tables::confirmation_height), account, value);
...
{
	return store.exists (transaction, store.table_to_column_family (tables::confirmation_height), account);
}

void nano::store::rocksdb::confirmation_height::del (store::write_transaction const & transaction, nano::account const & account)
{
	auto status = store.del (transaction, store.table_to_column_family (tables::confirmation_height), account);
	store.release_assert_success (status);
}
