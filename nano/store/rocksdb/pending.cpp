...
celerix::store::rocksdb::pending::pending (celerix::store::rocksdb::component & store) :
	store{ store } {};

void celerix::store::rocksdb::pending::put (store::write_transaction const & transaction, celerix::pending_key const & key, celerix::pending_info const & pending)
{
	auto status = store.put (transaction, tables::pending, key, pending);
	store.release_assert_success (status);
}

void celerix::store::rocksdb::pending::del (store::write_transaction const & transaction, celerix::pending_key const & key)
{
	auto status = store.del (transaction, tables::pending, key);
	store.release_assert_success (status);
}

std::optional<celerix::pending_info> celerix::store::rocksdb::pending::get (store::transaction const & transaction, celerix::pending_key const & key)
{
...
bool celerix::store::rocksdb::pending::exists (store::transaction const & transaction_a, celerix::pending_key const & key_a)
{
	auto iterator (begin (transaction_a, key_a));
	return iterator != end (transaction_a) && celerix::pending_key (iterator->first) == key_a;
}

bool celerix::store::rocksdb::pending::any (store::transaction const & transaction_a, celerix::account const & account_a)
{
...
auto celerix::store::rocksdb::pending::begin (store::transaction const & transaction_a, celerix::pending_key const & key_a) const -> iterator
{
...
auto celerix::store::rocksdb::pending::begin (store::transaction const & transaction_a) const -> iterator
{
...
auto celerix::store::rocksdb::pending::end (store::transaction const & transaction_a) const -> iterator
{
...
void celerix::store::rocksdb::pending::for_each_par (std::function<void (store::read_transaction const &, iterator, iterator)> const & action_a) const
{
