...
template class celerix::store::typed_iterator<celerix::account, celerix::account_info_v22>;

celerix::store::lmdb::component::component (celerix::logger & logger_a, std::filesystem::path const & path_a, celerix::ledger_constants & constants, celerix::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a, celerix::lmdb_config const & lmdb_config_a, bool backup_before_upgrade_a) :
	// clang-format off
	...
	logger{ logger_a },
	env (error, path_a, celerix::store::lmdb::env::options::make ().set_config (lmdb_config_a).set_use_no_mem_init (true)),
	...
	txn_tracking_enabled (txn_tracking_config_a.enable)
{
	if (!error)
	{
		...
		if (!is_fully_upgraded)
		{
			if (!is_fresh_db)
			{
				logger.info (celerix::log::type::lmdb, "Upgrade in progress...");
			...
			if (needs_vacuuming)
			{
				logger.info (celerix::log::type::lmdb, "Ledger vaccum in progress...");

				auto vacuum_success = vacuum_after_upgrade (path_a, lmdb_config_a);
				if (vacuum_success)
				{
					logger.info (celerix::log::type::lmdb, "Ledger vacuum completed");
				}
				else
				{
					logger.error (celerix::log::type::lmdb, "Ledger vaccum failed");
					logger.error (celerix::log::type::lmdb, "(Optional) Please ensure enough disk space is available for a copy of the database and try to vacuum after shutting down the node");
				}
			}
		}
		else
...
bool celerix::store::lmdb::component::vacuum_after_upgrade (std::filesystem::path const & path_a, celerix::lmdb_config const & lmdb_config_a)
{
	...
	if (vacuum_success)
	{
		...
		auto options = celerix::store::lmdb::env::options::make ()
...
void celerix::store::lmdb::component::serialize_mdb_tracker (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	mdb_txn_tracker.serialize_json (json, min_read_time, min_write_time);
}

void celerix::store::lmdb::component::serialize_memory_stats (boost::property_tree::ptree & json)
{
	MDB_stat stats;
...
celerix::store::write_transaction celerix::store::lmdb::component::tx_begin_write ()
{
	return env.tx_begin_write (create_txn_callbacks ());
}

celerix::store::read_transaction celerix::store::lmdb::component::tx_begin_read () const
{
	return env.tx_begin_read (create_txn_callbacks ());
}

std::string celerix::store::lmdb::component::vendor_get () const
{
	return boost::str (boost::format ("LMDB %1%.%2%.%3%") % MDB_VERSION_MAJOR % MDB_VERSION_MINOR % MDB_VERSION_PATCH);
}

celerix::store::lmdb::txn_callbacks celerix::store::lmdb::component::create_txn_callbacks () const
{
	celerix::store::lmdb::txn_callbacks mdb_txn_callbacks;
...
void celerix::store::lmdb::component::open_databases (bool & error_a, store::transaction const & transaction_a, unsigned flags)
{
...
bool celerix::store::lmdb::component::do_upgrades (store::write_transaction & transaction, celerix::ledger_constants & constants, bool & needs_vacuuming)
{
	auto error (false);
	auto version_l = version.get (transaction);
	if (version_l < version_minimum)
	{
		logger.critical (celerix::log::type::lmdb, "The version of the ledger ({}) is lower than the minimum ({}) which is supported for upgrades. Either upgrade a node first or delete the ledger.", version_l, version_minimum);
		return true;
	}
	switch (version_l)
	{
		...
		case 24:
			break;
		default:
			logger.critical (celerix::log::type::lmdb, "The version of the ledger ({}) is too high for this node", version_l);
			error = true;
			break;
	}
	return error;
...
void celerix::store::lmdb::component::upgrade_v21_to_v22 (store::write_transaction & transaction)
{
	logger.info (celerix::log::type::lmdb, "Upgrading database from v21 to v22...");
	...
	logger.info (celerix::log::type::lmdb, "Upgrading database from v21 to v22 completed");
}

// Fill rep_weights table with all existing representatives and their vote weight
void celerix::store::lmdb::component::upgrade_v22_to_v23 (store::write_transaction & transaction)
{
	logger.info (celerix::log::type::lmdb, "Upgrading database from v22 to v23...");
	...
	auto iterate_accounts = [this] (auto && func) {
		...
		auto it = typed_iterator<celerix::account, celerix::account_info_v22>{ store::iterator{ iterator::begin (env.tx (transaction), account_store.accounts_handle) } };
		auto const end = typed_iterator<celerix::account, celerix::account_info_v22>{ store::iterator{ iterator::end (env.tx (transaction), account_store.accounts_handle) } };
	...
	iterate_accounts ([this, &transaction, &processed] (celerix::account const & account, celerix::account_info_v22 const & account_info) {
		...
		{
			logger.info (celerix::log::type::lmdb, "Processed {} accounts", processed);
			transaction.refresh (); // Refresh to prevent excessive memory usage
		}
	});

	logger.info (celerix::log::type::lmdb, "Done processing {} accounts", processed);
	version.put (transaction, 23);

	logger.info (celerix::log::type::lmdb, "Upgrading database from v22 to v23 completed");
}

void celerix::store::lmdb::component::upgrade_v23_to_v24 (store::write_transaction & transaction)
{
	logger.info (celerix::log::type::lmdb, "Upgrading database from v23 to v24...");
	...
	logger.info (celerix::log::type::lmdb, "Upgrading database from v23 to v24 completed");
}

/** Takes a filepath, appends '_backup_<timestamp>' to the end (but before any extension) and saves that file in the same directory */
void celerix::store::lmdb::component::create_backup_file (celerix::store::lmdb::env & env_a, std::filesystem::path const & filepath_a, celerix::logger & logger)
{
	...
	logger.info (celerix::log::type::lmdb, "Performing {} backup before database upgrade...", filepath_a.filename ().string ());

	auto error (mdb_env_copy (env_a, backup_filepath.string ().c_str ()));
	if (error)
	{
		logger.critical (celerix::log::type::lmdb, "Database backup failed");
		std::exit (1);
	}
	else
	{
		logger.info (celerix::log::type::lmdb, "Database backup completed. Backup can be found at: {}", backup_filepath.string ());
	}
}

bool celerix::store::lmdb::component::exists (store::transaction const & transaction_a, tables table_a, celerix::store::lmdb::db_val const & key_a) const
{
	celerix::store::lmdb::db_val junk;
...
int celerix::store::lmdb::component::get (store::transaction const & transaction_a, tables table_a, celerix::store::lmdb::db_val const & key_a, celerix::store::lmdb::db_val & value_a) const
{
	return mdb_get (env.tx (transaction_a), table_to_dbi (table_a), key_a, value_a);
}

int celerix::store::lmdb::component::put (store::write_transaction const & transaction_a, tables table_a, celerix::store::lmdb::db_val const & key_a, celerix::store::lmdb::db_val const & value_a) const
{
	return (mdb_put (env.tx (transaction_a), table_to_dbi (table_a), key_a, value_a, 0));
}

int celerix::store::lmdb::component::del (store::write_transaction const & transaction_a, tables table_a, celerix::store::lmdb::db_val const & key_a) const
{
	return (mdb_del (env.tx (transaction_a), table_to_dbi (table_a), key_a, nullptr));
}

int celerix::store::lmdb::component::drop (store::write_transaction const & transaction_a, tables table_a)
{
	return clear (transaction_a, table_to_dbi (table_a));
}

int celerix::store::lmdb::component::clear (store::write_transaction const & transaction_a, MDB_dbi handle_a)
{
	return mdb_drop (env.tx (transaction_a), handle_a, 0);
}

uint64_t celerix::store::lmdb::component::count (store::transaction const & transaction_a, tables table_a) const
{
	return count (transaction_a, table_to_dbi (table_a));
}

uint64_t celerix::store::lmdb::component::count (store::transaction const & transaction_a, MDB_dbi db_a) const
{
	MDB_stat stats;
...
MDB_dbi celerix::store::lmdb::component::table_to_dbi (tables table_a) const
{
	switch (table_a)
	{
...
bool celerix::store::lmdb::component::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool celerix::store::lmdb::component::success (int status) const
{
	return (MDB_SUCCESS == status);
}

int celerix::store::lmdb::component::status_code_not_found () const
{
	return MDB_NOTFOUND;
}

std::string celerix::store::lmdb::component::error_string (int status) const
{
	return mdb_strerror (status);
}

bool celerix::store::lmdb::component::copy_db (std::filesystem::path const & destination_file)
{
	return !mdb_env_copy2 (env, destination_file.string ().c_str (), MDB_CP_COMPACT);
}

void celerix::store::lmdb::component::rebuild_db (store::write_transaction const & transaction_a)
{
	...
	for (auto const & table : tables)
	{
		...
		for (typed_iterator<celerix::uint256_union, celerix::store::lmdb::db_val> i{ store::iterator{ iterator::begin (env.tx (transaction_a), table) } }, n{ store::iterator{ iterator::end (env.tx (transaction_a), table) } }; i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), temp, celerix::store::lmdb::db_val (i->first), i->second, MDB_APPEND);
		...
		for (typed_iterator<celerix::uint256_union, celerix::store::lmdb::db_val> i{ store::iterator{ iterator::begin (env.tx (transaction_a), temp) } }, n{ store::iterator{ iterator::end (env.tx (transaction_a), temp) } }; i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), table, celerix::store::lmdb::db_val (i->first), i->second, MDB_APPEND);
			release_assert_success (s);
		}
		release_assert (count (transaction_a, table) == count (transaction_a, temp));
	...
	// Pending table
	{
		...
		mdb_dbi_open (env.tx (transaction_a), "temp_table", MDB_CREATE, &temp);
		// Copy all values to temporary table
		for (typed_iterator<celerix::pending_key, celerix::pending_info> i{ store::iterator{ iterator::begin (env.tx (transaction_a), pending_store.pending_handle) } }, n{ store::iterator{ iterator::end (env.tx (transaction_a), pending_store.pending_handle) } }; i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), temp, celerix::store::lmdb::db_val (i->first), celerix::store::lmdb::db_val (i->second), MDB_APPEND);
			release_assert_success (s);
		}
		...
		// Put values from copy
		for (typed_iterator<celerix::pending_key, celerix::pending_info> i{ store::iterator{ iterator::begin (env.tx (transaction_a), temp) } }, n{ store::iterator{ iterator::end (env.tx (transaction_a), temp) } }; i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), pending_store.pending_handle, celerix::store::lmdb::db_val (i->first), celerix::store::lmdb::db_val (i->second), MDB_APPEND);
			release_assert_success (s);
		}
		release_assert (count (transaction_a, pending_store.pending_handle) == count (transaction_a, temp));
		mdb_drop (env.tx (transaction_a), temp, 1);
...
bool celerix::store::lmdb::component::init_error () const
{
	return error;
}

celerix::store::lmdb::component::upgrade_counters::upgrade_counters (uint64_t count_before_v0, uint64_t count_before_v1) :
	before_v0 (count_before_v0),
	before_v1 (count_before_v1)
{
}

bool celerix::store::lmdb::component::upgrade_counters::are_equal () const
{
	return (before_v0 == after_v0) && (before_v1 == after_v1);
}

unsigned celerix::store::lmdb::component::max_block_write_batch_num () const
{
