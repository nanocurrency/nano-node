...
{
public:
	explicit matches_txn (celerix::store::transaction_impl const * transaction_impl_a) :
		transaction_impl (transaction_impl_a)
	{
	}

	bool operator() (celerix::mdb_txn_stats const & mdb_txn_stats)
	{
		return (mdb_txn_stats.transaction_impl == transaction_impl);
	}

private:
	celerix::store::transaction_impl const * transaction_impl;
};
}

celerix::store::lmdb::read_transaction_impl::read_transaction_impl (celerix::store::lmdb::env const & environment_a, celerix::store::lmdb::txn_callbacks txn_callbacks_a) :
...
celerix::store::lmdb::read_transaction_impl::~read_transaction_impl ()
{
...
void celerix::store::lmdb::read_transaction_impl::reset ()
{
	mdb_txn_reset (handle);
	txn_callbacks.txn_end (this);
}

void celerix::store::lmdb::read_transaction_impl::renew ()
{
...
void * celerix::store::lmdb::read_transaction_impl::get_handle () const
{
	return handle;
}

celerix::store::lmdb::write_transaction_impl::write_transaction_impl (celerix::store::lmdb::env const & environment_a, celerix::store::lmdb::txn_callbacks txn_callbacks_a) :
...
celerix::store::lmdb::write_transaction_impl::~write_transaction_impl ()
{
	commit ();
}

void celerix::store::lmdb::write_transaction_impl::commit ()
{
	if (active)
	{
		auto status = mdb_txn_commit (handle);
...
void celerix::store::lmdb::write_transaction_impl::renew ()
{
...
void * celerix::store::lmdb::write_transaction_impl::get_handle () const
{
	return handle;
}

bool celerix::store::lmdb::write_transaction_impl::contains (celerix::tables table_a) const
{
	// LMDB locks on every write
	return true;
}

celerix::mdb_txn_tracker::mdb_txn_tracker (celerix::logger & logger_a, celerix::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a) :
	logger (logger_a),
...
void celerix::mdb_txn_tracker::serialize_json (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	...
	for (std::size_t i = 0; i < times_since_start.size (); ++i)
	{
		...
		{
			celerix::jsonconfig mdb_lock_config;
			...
			celerix::jsonconfig stack (stacktrace_config);
...
void celerix::mdb_txn_tracker::log_if_held_long_enough (celerix::mdb_txn_stats const & mdb_txn_stats) const
{
	...
	bool is_blk_processing_thread = mdb_txn_stats.thread_name == celerix::thread_role::get_string (celerix::thread_role::name::block_processing);
	...
	if (!should_ignore && ((is_write && time_open >= txn_tracking_config.min_write_txn_time) || (!is_write && time_open >= txn_tracking_config.min_read_txn_time)))
	{
		...
		logger.warn (celerix::log::type::txn_tracker, "{}ms {} held on thread {}\n{}",
		time_open.count (),
		is_write ? "write lock" : "read",
		mdb_txn_stats.thread_name,
		celerix::util::to_str (*mdb_txn_stats.stacktrace));
	}
}

void celerix::mdb_txn_tracker::add (store::transaction_impl const * transaction_impl)
{
	celerix::lock_guard<celerix::mutex> guard (mutex);
...
void celerix::mdb_txn_tracker::erase (store::transaction_impl const * transaction_impl)
{
...
celerix::mdb_txn_stats::mdb_txn_stats (store::transaction_impl const * transaction_impl) :
	transaction_impl (transaction_impl),
	thread_name (celerix::thread_role::get_string ()),
	stacktrace (std::make_shared<boost::stacktrace::stacktrace> ())
{
	timer.start ();
}

bool celerix::mdb_txn_stats::is_write () const
{
