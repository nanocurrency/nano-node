...
{
	increment (transaction_a, tables::cached_counts, rocksdb_val (rocksdb::Slice (table_to_column_family (tables::blocks)->GetName ())), num_a);
}

void nano::rocksdb_store::decrement_state_block_count (nano::write_transaction const & transaction_a, uint64_t num_a)
{
	decrement (transaction_a, tables::cached_counts, rocksdb_val (rocksdb::Slice (table_to_column_family (tables::blocks)->GetName ())), num_a);
}
