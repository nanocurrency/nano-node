...
void nano::scheduler::buckets::push (uint64_t time, std::shared_ptr<nano::block> block, nano::amount const & priority)
{
	...
	if (bucket.queue.size () > std::max (decltype (maximum){ 1 }, maximum / buckets_m.size ()))
	{
...
{
	debug_assert (!empty ());
	debug_assert (!buckets_m[*current].queue.empty ());
	auto result = buckets_m[*current].queue.begin ()->block;
	return result;
}
...
{
	return buckets_m[index].queue.size ();
}
