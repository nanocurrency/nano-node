...
void nano::bootstrap_attempt_lazy::lazy_pull_flush (nano::unique_lock<std::mutex> & lock_a)
{
	...
	if (pulling < max_pulls)
	{
		...
		while (!lazy_pulls.empty () && count < max_pulls)
		{
			...
			if (!lazy_blocks_processed (pull_start.first) && !node->ledger.block_or_pruned_exists (transaction, pull_start.first))
			{
				lock_a.unlock ();
...
void nano::bootstrap_attempt_lazy::lazy_block_state (std::shared_ptr<nano::block> block_a, unsigned retry_limit)
{
	...
	if (block_l != nullptr)
	{
		...
		if (!link.is_zero () && !node->ledger.is_epoch_link (link) && !lazy_blocks_processed (link) && !node->ledger.block_or_pruned_exists (transaction, link))
		{
