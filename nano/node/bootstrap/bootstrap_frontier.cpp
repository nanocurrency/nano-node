...
void nano::frontier_req_server::next ()
{
	...
	if (accounts.empty ())
	{
		...
		{
			auto state (connection->node->ledger.account_state (transaction, i->second));
			if (!skip_old || (now - state.modified ()) <= request->age)
			{
				nano::account const & account (i->first);
				accounts.emplace_back (account, state.head ());
			}
		}
		/* If loop breaks before max_size, then latest_end () is reached
		Add empty record to finish frontier_req_server */
