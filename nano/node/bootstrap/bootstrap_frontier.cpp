...
void nano::frontier_req_client::next ()
{
	...
	if (accounts.empty ())
	{
		...
		for (auto i (connection->node->store.account.accounts_begin (transaction, current.number () + 1)), n (connection->node->store.account.accounts_end ()); i != n && accounts.size () != max_size; ++i)
		{
