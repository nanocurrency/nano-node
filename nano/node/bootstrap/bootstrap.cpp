...
{
	nano::lock_guard lock (observers_mutex);
...
std::unique_ptr<nano::container_info_component> nano::collect_container_info (bootstrap_initiator & bootstrap_initiator, const std::string & name)
{
	...
	size_t cache_count;
	{
		nano::lock_guard guard (bootstrap_initiator.observers_mutex);
		count = bootstrap_initiator.observers.size ();
	}
	{
		nano::lock_guard guard (bootstrap_initiator.cache.pulls_cache_mutex);
...
void nano::pulls_cache::add (nano::pull_info const & pull_a)
{
	...
	{
		nano::lock_guard guard (pulls_cache_mutex);
		// Clean old pull
...
{
	nano::lock_guard lock (bootstrap_attempts_mutex);
