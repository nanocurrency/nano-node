...
namespace ipc
{
	...
	public:
		...
		virtual void set_service_name (std::string const &) = 0;
	...
	{
	public:
		subscription (std::weak_ptr<nano::ipc::subscriber> subscriber_a, std::shared_ptr<TopicType> const & topic_a) :
	...
	public:
		...
		void subscribe (std::weak_ptr<nano::ipc::subscriber> subscriber_a, std::shared_ptr<nanoapi::TopicConfirmationT> const & confirmation_a);
		/** Subscribe to EventServiceStop notifications for \p subscriber_a. The subscriber must first have called ServiceRegister. */
		void subscribe (std::weak_ptr<nano::ipc::subscriber> subscriber_a, std::shared_ptr<nanoapi::TopicServiceStopT> const & service_stop_a);
		...
		void service_register (std::string service_name_a, std::weak_ptr<nano::ipc::subscriber> subscriber_a);
		/** Sends a notification to the session associated with the given service (if the session has subscribed to TopicServiceStop) */
		void service_stop (std::string service_name_a);
