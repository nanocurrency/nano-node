...
class stream_wrapper : public celerix::websocket::websocket_stream_concept
{
public:
...
#ifdef CELERIX_SECURE_RPC
celerix::websocket::stream::stream (socket_type socket_a, boost::asio::ssl::context & ctx_a)
{
	impl = std::make_unique<stream_wrapper<wss_type>> (std::move (socket_a), ctx_a);
}
#endif
celerix::websocket::stream::stream (socket_type socket_a)
{
	impl = std::make_unique<stream_wrapper<ws_type>> (std::move (socket_a));
}

[[nodiscard]] boost::asio::strand<boost::asio::io_context::executor_type> & celerix::websocket::stream::get_strand ()
{
	return impl->get_strand ();
}

[[nodiscard]] socket_type & celerix::websocket::stream::get_socket ()
{
	return impl->get_socket ();
}

void celerix::websocket::stream::handshake (std::function<void (boost::system::error_code const & ec)> callback_a)
{
	impl->handshake (callback_a);
}

void celerix::websocket::stream::close (boost::beast::websocket::close_reason const & reason_a, boost::system::error_code & ec_a)
{
	impl->close (reason_a, ec_a);
}

void celerix::websocket::stream::async_write (celerix::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code, std::size_t)> callback_a)
{
	impl->async_write (buffer_a, callback_a);
}

void celerix::websocket::stream::async_read (boost::beast::multi_buffer & buffer_a, std::function<void (boost::system::error_code, std::size_t)> callback_a)
{
