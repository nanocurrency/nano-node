...
void nano::mdb_store::upgrade_v1_to_v2 (nano::write_transaction const & transaction_a)
{
	...
	while (!account.is_zero ())
	{
		...
		std::cerr << std::hex;
		if (i != nano::mdb_iterator<nano::uint256_union, nano::account_info_v1>{})
		{
...
nano::summation_visitor_v14::summation_visitor_v14 (nano::transaction const & transaction_a, nano::mdb_store const & store_a) :
transaction (transaction_a),
store (store_a)
{
}

void nano::summation_visitor_v14::send_block (nano::send_block const & block_a)
{
...
void nano::summation_visitor_v14::state_block (nano::state_block const & block_a)
{
	assert (current->type != summation_type::invalid && current != nullptr);
	sum_set (block_a.hashables.balance.number ());
	if (current->type == summation_type::amount)
...
void nano::summation_visitor_v14::receive_block (nano::receive_block const & block_a)
{
	assert (current->type != summation_type::invalid && current != nullptr);
	if (current->type == summation_type::amount)
	{
...
void nano::summation_visitor_v14::open_block (nano::open_block const & block_a)
{
	assert (current->type != summation_type::invalid && current != nullptr);
	if (current->type == summation_type::amount)
	{
...
void nano::summation_visitor_v14::change_block (nano::change_block const & block_a)
{
	assert (current->type != summation_type::invalid && current != nullptr);
	if (current->type == summation_type::amount)
	{
...
nano::summation_visitor_v14::frame nano::summation_visitor_v14::push (nano::summation_visitor_v14::summation_type type_a, nano::block_hash const & hash_a)
{
	frames.emplace (type_a, type_a == summation_type::balance ? hash_a : 0, type_a == summation_type::amount ? hash_a : 0);
	return frames.top ();
}

void nano::summation_visitor_v14::sum_add (nano::uint128_t addend_a)
{
	current->sum += addend_a;
	result = current->sum;
}

void nano::summation_visitor_v14::sum_set (nano::uint128_t value_a)
{
	current->sum = value_a;
	result = current->sum;
}

nano::uint128_t nano::summation_visitor_v14::compute_internal (nano::summation_visitor_v14::summation_type type_a, nano::block_hash const & hash_a)
{
	push (type_a, hash_a);
	...
	while (!frames.empty ())
	{
		...
		if (current->type == summation_type::balance)
		{
			...
			while (!current->awaiting_result && (!current->balance_hash.is_zero () || !current->amount_hash.is_zero ()))
			{
				...
				{
					auto block (store.block_get_v14 (transaction, current->balance_hash));
					assert (block != nullptr);
					block->visit (*this);
				}
			}
		...
		else if (current->type == summation_type::amount)
		{
			...
			while (!current->awaiting_result && (!current->amount_hash.is_zero () || !current->balance_hash.is_zero ()))
			{
				if (!current->amount_hash.is_zero ())
				{
					auto block (store.block_get_v14 (transaction, current->amount_hash));
					if (block != nullptr)
					{
						block->visit (*this);
					}
...
void nano::summation_visitor_v14::epilogue ()
{
	if (!current->awaiting_result)
	{
		frames.pop ();
...
nano::uint128_t nano::summation_visitor_v14::compute_amount (nano::block_hash const & block_hash)
{
	return compute_internal (summation_type::amount, block_hash);
}

nano::uint128_t nano::summation_visitor_v14::compute_balance (nano::block_hash const & block_hash)
{
...
{
	assert (!full_sideband (transaction_a));
	summation_visitor_v14 visitor (transaction_a, *this);
