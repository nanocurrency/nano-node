...
void nano::wallet::work_ensure (nano::account const & account_a, nano::root const & root_a)
{
	...
	wallets.node.workers.add_delayed_task (std::chrono::steady_clock::now () + precache_delay, [this_l = shared_from_this (), account_a, root_a] {
...
void nano::work_watcher::watching (nano::qualified_root const & root_a, std::shared_ptr<nano::state_block> block_a)
{
	...
	node.workers.add_delayed_task (std::chrono::steady_clock::now () + node.config.work_watcher_period, [block_a, root_a, watcher_w]() {
...
void nano::wallets::ongoing_compute_reps ()
{
	...
	node.workers.add_delayed_task (std::chrono::steady_clock::now () + compute_delay, [&node_l]() {
