...
nano::node_flags::node_flags (boost::program_options::variables_map const & vm) :
disable_backup (vm.count ("disable_backup") > 0),
disable_lazy_bootstrap (vm.count ("disable_lazy_bootstrap") > 0),
disable_legacy_bootstrap (vm.count ("disable_legacy_bootstrap") > 0),
disable_wallet_bootstrap (vm.count ("disable_wallet_bootstrap") > 0),
disable_bootstrap_listener (vm.count ("disable_bootstrap_listener") > 0),
disable_unchecked_cleanup (vm.count ("disable_unchecked_cleanup") > 0),
disable_unchecked_drop (vm.count ("disable_unchecked_drop") > 0),
fast_bootstrap (vm.count ("fast_bootstrap") > 0)
{
	if (fast_bootstrap)
	{
		block_processor_batch_size = 256 * 1024;
		block_processor_full_size = 1024 * 1024;
		block_processor_verification_size = std::numeric_limits<size_t>::max ();
	}
	auto batch_size_it = vm.find ("batch_size");
	if (batch_size_it != vm.end ())
	{
		sideband_batch_size = batch_size_it->second.as<size_t> ();
	}
	auto block_processor_batch_size_it = vm.find ("block_processor_batch_size");
	if (block_processor_batch_size_it != vm.end ())
	{
		block_processor_batch_size = block_processor_batch_size_it->second.as<size_t> ();
	}
	auto block_processor_full_size_it = vm.find ("block_processor_full_size");
	if (block_processor_full_size_it != vm.end ())
	{
		block_processor_full_size = block_processor_full_size_it->second.as<size_t> ();
	}
	...
	{
		block_processor_verification_size = block_processor_verification_size_it->second.as<size_t> ();
	}
}
