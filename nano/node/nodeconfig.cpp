...
nano::error nano::node_config::serialize_toml (nano::tomlconfig & toml) const
{
	...
	toml.put ("wallet_threads", wallet_threads, "Number of threads dedicated to wallet background work.\ntype:uint64");
...
nano::error nano::node_config::deserialize_toml (nano::tomlconfig & toml)
{
	try
	{
		...
		toml.get<unsigned> ("wallet_threads", wallet_threads);
		...
		toml.get<bool> ("extended_ledger_index", extended_ledger_index);
