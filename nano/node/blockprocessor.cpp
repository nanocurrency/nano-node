...
state_block_signature_verification (node.ledger, node.checker, node.network_params.ledger.epochs, node.config, node.logger, node.flags.block_processor_verification_size)
{
...
void nano::block_processor::add (nano::unchecked_info const & info_a, const bool push_front_preference_a)
{
	...
	debug_assert (nano::simple_block_validation (info_a.block.get (), node.network_params.ledger.epochs) == nano::error_blocks::none);
...
nano::process_return nano::block_processor::process_one (nano::write_transaction const & transaction_a, block_post_events & events_a, nano::unchecked_info info_a, const bool watch_work_a, const bool forced_a, nano::block_origin const origin_a)
{
	...
	switch (result.code)
	{
		...
		case nano::process_result::gap_previous:
			...
			node.store.unchecked_put (transaction_a, nano::unchecked_key (block->previous (), hash), info_a);
		...
		case nano::process_result::gap_source:
			...
			node.store.unchecked_put (transaction_a, nano::unchecked_key (node.ledger.block_source (transaction_a, *(block)), hash), info_a);
