	...
	for (auto const & [hash, root] : requests_a)
	{
		...
		else
		{
			...
			if (!final_vote_hashes.empty ())
			{
				...
				block = ledger->get (transaction, final_vote_hashes[0]);
				// Allow same root vote
				if (block != nullptr && final_vote_hashes.size () > 1)
				{
					to_generate_final.push_back (block);
					block = ledger->get (transaction, final_vote_hashes[1]);
			...
			if (block == nullptr)
			{
				block = ledger->get (transaction, hash);
			...
			if (block == nullptr && !root.is_zero ())
			{
				...
				if (successor)
				{
					auto successor_block = ledger->get (transaction, successor.value ());
