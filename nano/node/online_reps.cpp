...
void nano::online_reps::sample ()
{
	...
	nano::uint128_t trend_l;
	{
		...
		while (ledger.store.online_weight.online_weight_count (transaction) >= config.network_params.node.max_weight_samples)
		{
			auto oldest (ledger.store.online_weight.online_weight_begin (transaction));
			debug_assert (oldest != ledger.store.online_weight.online_weight_end ());
			ledger.store.online_weight.online_weight_del (transaction, oldest->first);
		}
		ledger.store.online_weight.online_weight_put (transaction, std::chrono::system_clock::now ().time_since_epoch ().count (), online_l);
...
nano::uint128_t nano::online_reps::calculate_trend (nano::transaction & transaction_a) const
{
	...
	for (auto i (ledger.store.online_weight.online_weight_begin (transaction_a)), n (ledger.store.online_weight.online_weight_end ()); i != n; ++i)
	{
