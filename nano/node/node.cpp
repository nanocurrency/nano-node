			...
			if (active.insert (ledger_block, false, [this_w, root](std::shared_ptr<nano::block>) {
				          if (auto this_l = this_w.lock ())
				          {
					          ...
					          if (attempt && attempt->mode == nano::bootstrap_mode::legacy)
					          {
						          ...
						          if (!account.is_zero ())
						          {
							          this_l->bootstrap_initiator.connections->requeue_pull (nano::pull_info (account, root, root));
						          }
						          else if (this_l->ledger.store.account_exists (transaction, root))
						          {
							          this_l->bootstrap_initiator.connections->requeue_pull (nano::pull_info (root, nano::block_hash (0), nano::block_hash (0)));
						          }
					          }
				          }
			          })
