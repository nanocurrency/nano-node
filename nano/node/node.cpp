...
store_impl (std::make_unique<nano::mdb_store> (logger, application_path_a / "data.ldb", config_a.diagnostics_config.txn_tracking, config_a.block_processor_batch_max_time, config_a.lmdb_max_dbs, !flags.disable_unchecked_drop, flags.sideband_batch_size, config_a.backup_before_upgrade)),
store (*store_impl),
			...
			if (!nano::try_read (weight_stream, block_height))
			{
				...
				if (ledger.store.block_count (transaction).sum () < max_blocks)
				{
					...
					while (true)
					{
						...
						ledger.set_bootstrap_weight (account, weight.number ());
					}
				}
			}
		}
...
std::unique_ptr<nano::block_store> nano::make_store (nano::logger_mt & logger, boost::filesystem::path const & path)
{
	return std::make_unique<nano::mdb_store> (logger, path);
}
