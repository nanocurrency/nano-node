...
void nano::node::long_inactivity_cleanup ()
{
	...
	if (store.online_weight.online_weight_count (transaction) > 0)
	{
		auto sample (store.online_weight.online_weight_rbegin (transaction));
		auto n (store.online_weight.online_weight_end ());
	...
	if (perform_cleanup)
	{
		store.online_weight.online_weight_clear (transaction);
	...
	*/
	if (bootstrap_weight_reached)
	{
		if (warmed_up < 3)
		{
			...
			auto last_record = store.online_weight.online_weight_rbegin (store.tx_begin_read ());
			if (last_record != store.online_weight.online_weight_end ())
			{
