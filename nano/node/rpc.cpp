...
void nano::rpc_handler::version ()
{
	...
	{
		response_l.put ("node_vendor", boost::str (boost::format ("Banano %1%") % NANO_MAJOR_MINOR_VERSION));
	}
	else
	{
		response_l.put ("node_vendor", boost::str (boost::format ("Banano %1%") % NANO_MAJOR_MINOR_RC_VERSION));
	}
	response_errors ();
}
