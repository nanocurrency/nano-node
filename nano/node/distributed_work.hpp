...
{
public:
	distributed_work (unsigned int, nano::node &, nano::root const &, std::function<void(boost::optional<uint64_t>)> const &, uint64_t);
	~distributed_work ();
	void start ();
	void start_work ();
...
public:
	...
	void make (nano::root const &, std::function<void(boost::optional<uint64_t>)> const &, uint64_t);
	void make (unsigned int, nano::root const &, std::function<void(boost::optional<uint64_t>)> const &, uint64_t);
