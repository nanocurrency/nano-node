...
void nano::transport::tcp_channels::list (std::deque<std::shared_ptr<nano::transport::channel>> & deque_a, bool include_server_channels_a, uint8_t minimum_version_a)
{
	nano::lock_guard<std::mutex> lock (mutex);
	// clang-format off
	nano::transform_if (channels.begin (), channels.end (), std::back_inserter (deque_a),
		[include_server_channels_a, minimum_version_a](auto & channel_a) { return channel_a.channel->get_network_version () >= minimum_version_a && (include_server_channels_a || !channel_a.channel->server); },
