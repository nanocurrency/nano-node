...
celerix::transport::inproc::channel::channel (celerix::node & node, celerix::node & destination) :
 ...
 */
bool celerix::transport::inproc::channel::send_buffer (celerix::shared_const_buffer const & buffer, celerix::transport::traffic_type traffic_type, celerix::transport::channel::callback_t callback)
{
	std::size_t offset{ 0 };
	...
	auto const message_deserializer = std::make_shared<celerix::transport::message_deserializer> (node.network_params.network, node.network.filter, node.block_uniquer, node.vote_uniquer, buffer_read_fn);
	message_deserializer->read (
	[this] (boost::system::error_code ec_a, std::unique_ptr<celerix::message> message_a) {
		if (ec_a || !message_a)
		{
			return;
		}
		...
		auto remote_channel = std::make_shared<celerix::transport::inproc::channel> (destination, node);

		// process message
		{
			node.stats.inc (celerix::stat::type::message, to_stat_detail (message_a->type ()), celerix::stat::dir::in);
...
std::string celerix::transport::inproc::channel::to_string () const
{
