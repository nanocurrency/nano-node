...
usage() {
	printf "Usage:\n"
	printf "  $0 celerix_node [daemon] [cli_options] [-l] [-v size]\n"
	printf "    daemon\n"
	printf "      start as daemon\n\n"
	printf "    cli_options\n"
	printf "      celerix_node cli options <see celerix_node --help>\n\n"
	printf "    -l\n"
	...
	printf "      usage\n\n"
	printf "default:\n"
	printf "  $0 celerix_node daemon -l\n"
}
...
if [ "$1" = 'celerix_node' ]; then
	command="${command}celerix_node"
	shift
	for i in $@; do
		case $i in
		"daemon")
...
network="$(cat /etc/celerix-network)"
case "${network}" in
live | '')
	network='live'
	dirSuffix=''
...
celerixdir="${HOME}/Celerix${dirSuffix}"
dbFile="${celerixdir}/data.ldb"

if [ -d "${raidir}" ]; then
	echo "Moving ${raidir} to ${celerixdir}"
	mv "$raidir" "$celerixdir"
else
	mkdir -p "${celerixdir}"
fi

if [ ! -f "${celerixdir}/config-node.toml" ] && [ ! -f "${celerixdir}/config.json" ]; then
	echo "Config file not found, adding default."
	cp "/usr/share/celerix/config/config-node.toml" "${celerixdir}/config-node.toml"
	cp "/usr/share/celerix/config/config-rpc.toml" "${celerixdir}/config-rpc.toml"
fi
