...
#ifdef BANANO_SECURE_RPC
#include <banano/node/rpc_secure.hpp>
#endif
...
void rai::rpc_handler::banoshi_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	rai::uint128_union amount;
	if (!amount.decode_dec (amount_text))
	{
		auto result (amount.number () / rai::banoshi_ratio);
		boost::property_tree::ptree response_l;
		response_l.put ("amount", result.convert_to<std::string> ());
		response (response_l);
	}
...
void rai::rpc_handler::banoshi_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	rai::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void rai::rpc_handler::raw_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	rai::uint128_union amount;
	if (!amount.decode_dec (amount_text))
	{
		auto result (amount.number () / rai::RAW_ratio);
		boost::property_tree::ptree response_l;
		response_l.put ("amount", result.convert_to<std::string> ());
		response (response_l);
	}
...
void rai::rpc_handler::raw_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	rai::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void rai::rpc_handler::version ()
{
	...
	response_l.put ("node_vendor", boost::str (boost::format ("Banano %1%.%2%") % BANANO_VERSION_MAJOR % BANANO_VERSION_MINOR));
	response (response_l);
}
...
void rai::rpc_handler::process_request ()
{
	try
	{
		...
		else if (action == "banoshi_from_raw")
		{
			banoshi_from_raw ();
		}
		else if (action == "banoshi_to_raw")
		{
			banoshi_to_raw ();
		}
