...
void rai::rpc_handler::ban_from_raw (rai::uint128_t ratio)
{
...
void rai::rpc_handler::ban_to_raw (rai::uint128_t ratio)
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void rai::rpc_handler::version ()
{
	...
	response_l.put ("node_vendor", boost::str (boost::format ("Banano %1%.%2%") % RAIBLOCKS_VERSION_MAJOR % RAIBLOCKS_VERSION_MINOR));
	response_errors ();
}
...
void rai::rpc_handler::process_request ()
{
	try
	{
		...
		else
		{
			...
			{
				ban_from_raw (rai::banoshi_ratio);
			}
			else if (action == "banoshi_to_raw")
			{
				ban_to_raw (rai::banoshi_ratio);
			}
