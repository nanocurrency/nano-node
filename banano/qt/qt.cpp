...
bool banano_qt::eventloop_processor::event (QEvent * event_a)
{
	assert (dynamic_cast<banano_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<banano_qt::eventloop_event *> (event_a)->action ();
	return true;
}

banano_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

banano_qt::self_pane::self_pane (banano_qt::wallet & wallet_a, rai::account const & account_a) :
window (new QWidget),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("Version %1%.%2%") % BANANO_VERSION_MAJOR % BANANO_VERSION_MINOR).c_str ());
...
void banano_qt::self_pane::refresh_balance ()
{
...
banano_qt::accounts::accounts (banano_qt::wallet & wallet_a) :
window (new QWidget),
...
banano_qt::import::import (banano_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
banano_qt::block_viewer::block_viewer (banano_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
banano_qt::account_viewer::account_viewer (banano_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void banano_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
banano_qt::status::status (banano_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (banano_qt::status_types::nominal);
	set_text ();
}

void banano_qt::status::erase (banano_qt::status_types status_a)
{
	assert (status_a != banano_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void banano_qt::status::insert (banano_qt::status_types status_a)
{
	assert (status_a != banano_qt::status_types::nominal);
...
std::string banano_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case banano_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case banano_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case banano_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case banano_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case banano_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case banano_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case banano_qt::status_types::nominal:
...
std::string banano_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
banano_qt::wallet::wallet (QApplication & application_a, banano_qt::eventloop_processor & processor_a, rai::node & node_a, std::shared_ptr<rai::wallet> wallet_a, rai::account & account_a) :
rendering_ratio (rai::BAN_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
void banano_qt::wallet::start ()
{
	std::weak_ptr<banano_qt::wallet> this_w (shared_from_this ());
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (banano_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (banano_qt::status_types::active);
					}
				}
			}));
		}
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (banano_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (banano_qt::status_types::synchronizing);
					}
				}
			}));
		}
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (banano_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (banano_qt::status_types::working);
					}
				}
			}));
		}
...
void banano_qt::wallet::update_connected ()
{
	if (node.peers.empty ())
	{
		active_status.insert (banano_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (banano_qt::status_types::disconnected);
	}
}
...
void banano_qt::wallet::change_rendering_ratio (rai::uint128_t const & rendering_ratio_a)
{
...
std::string banano_qt::wallet::format_balance (rai::uint128_t const & balance) const
{
	auto balance_str = rai::amount (balance).format_balance (rendering_ratio, 0, false);
	auto unit = std::string ("BAN");
	if (rendering_ratio == rai::banoshi_ratio)
	{
...
void banano_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void banano_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

banano_qt::settings::settings (banano_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
void banano_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void banano_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (banano_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (banano_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (banano_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (banano_qt::status_types::vulnerable);
	}
}

banano_qt::advanced_actions::advanced_actions (banano_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
bananobutton (new QRadioButton ("BANANO")),
banoshibutton (new QRadioButton ("banoshi")),
...
wallet (wallet_a)
{
	ratio_group->addButton (bananobutton);
	ratio_group->addButton (banoshibutton);
	ratio_group->addButton (rawbutton);
	ratio_group->setId (bananobutton, 0);
	ratio_group->setId (banoshibutton, 1);
	ratio_group->setId (rawbutton, 2);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (bananobutton);
	...
	QObject::connect (bananobutton, &QRadioButton::toggled, [this]() {
		if (bananobutton->isChecked ())
		{
...
void banano_qt::advanced_actions::refresh_peers ()
{
...
void banano_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

banano_qt::block_entry::block_entry (banano_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
banano_qt::block_creation::block_creation (banano_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void banano_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void banano_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void banano_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void banano_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void banano_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
...
void banano_qt::block_creation::create_send ()
{
	rai::account account_l;
...
void banano_qt::block_creation::create_receive ()
{
...
void banano_qt::block_creation::create_change ()
{
	rai::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
...
void banano_qt::block_creation::create_open ()
{
	rai::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
