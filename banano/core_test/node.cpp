...
{
	rai::system system (24000, 1);
	auto price1 (system.nodes[0]->price (rai::kBAN_ratio, 1));
	ASSERT_EQ (rai::node::price_max * 100.0, price1);
	auto price2 (system.nodes[0]->price (rai::kBAN_ratio * int(rai::node::free_cutoff + 1), 1));
	ASSERT_EQ (0, price2);
	auto price3 (system.nodes[0]->price (rai::kBAN_ratio * int(rai::node::free_cutoff + 2) / 2, 1));
	ASSERT_EQ (rai::node::price_max * 100.0 / 2, price3);
	auto price4 (system.nodes[0]->price (rai::kBAN_ratio * int(rai::node::free_cutoff) * 2, 1));
	ASSERT_EQ (0, price4);
}
...
TEST (node_config, unversioned_v2_upgrade)
{
	...
	ASSERT_EQ ("ban_3e3j5tkog48pnny9dmfzj1r16pg8t1e76dz5tmac6iq689wyjfpiij4txtdo", config1.preconfigured_representatives[0].to_account ());
	auto reps (tree.get_child ("preconfigured_representatives"));
	ASSERT_EQ (1, reps.size ());
	ASSERT_EQ ("ban_3e3j5tkog48pnny9dmfzj1r16pg8t1e76dz5tmac6iq689wyjfpiij4txtdo", reps.begin ()->second.get<std::string> (""));
...
TEST (node, DISABLED_fork_bootstrap_flip)
{
	...
	rai::keypair key1;
	auto send1 (std::make_shared<rai::send_block> (latest, key1.pub, rai::genesis_amount - rai::kBAN_ratio, rai::test_genesis_key.prv, rai::test_genesis_key.pub, system0.work.generate (latest)));
	rai::keypair key2;
...
TEST (node, broadcast_elected)
{
	...
	//std::cerr << "Other: " << rep_other.pub.to_account () << std::endl;
	{
		...
		rai::send_block fund_big (node0->ledger.latest (transaction0, rai::test_genesis_key.pub), rep_big.pub, rai::kBAN_ratio * 5, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
		rai::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
		rai::send_block fund_small (fund_big.hash (), rep_small.pub, rai::kBAN_ratio * 2, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
		rai::open_block open_small (fund_small.hash (), rep_small.pub, rep_small.pub, rep_small.prv, rep_small.pub, 0);
		rai::send_block fund_other (fund_small.hash (), rep_other.pub, rai::kBAN_ratio * 1, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
...
TEST (node, DISABLED_unconfirmed_send)
{
	...
	rai::send_block send2 (latest, rai::genesis_account, rai::BAN_ratio, key0.prv, key0.pub, node0.generate_work (latest));
	{
