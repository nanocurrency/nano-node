...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path)
{
	...
	if (!error)
	{
		...
		std::shared_ptr<banano_qt::wallet> gui;
		...
		if (!init.error ())
		{
			...
			application.postEvent (&processor, new banano_qt::eventloop_event ([&]() {
				gui = std::make_shared<banano_qt::wallet> (application, processor, *node, wallet, config.account);
				splash->close ();
				gui->start ();
