...
TEST (interface, sign_transaction)
{
	...
	galileo::send_block send (0, 0, 0, key, pub, 0);
	ASSERT_FALSE (galileo::validate_message (pub, send.hash (), send.signature));
	send.signature.bytes[0] ^= 1;
	ASSERT_TRUE (galileo::validate_message (pub, send.hash (), send.signature));
	...
	auto send1 (dynamic_cast<galileo::send_block *> (block.get ()));
	ASSERT_NE (nullptr, send1);
	ASSERT_FALSE (galileo::validate_message (pub, send.hash (), send1->signature));
	free (transaction);
}

TEST (interface, fail_sign_transaction)
{
	galileo::uint256_union data (0);
...
TEST (interface, work_transaction)
{
	...
	galileo::uint256_union pub;
	xrb_key_account (key.data.bytes.data (), pub.bytes.data ());
	galileo::send_block send (1, 0, 0, key, pub, 0);
