...
int main (int argc, char ** argv)
{
	...
	static int count (16);
	galileo::system system (24000, count);
	std::unique_ptr<QTabWidget> client_tabs (new QTabWidget);
	std::vector<std::unique_ptr<galileo_qt::wallet>> guis;
	for (auto i (0); i < count; ++i)
	{
		galileo::uint256_union wallet_id;
		galileo::random_pool.GenerateBlock (wallet_id.bytes.data (), wallet_id.bytes.size ());
		auto wallet (system.nodes[i]->wallets.create (wallet_id));
		galileo::keypair key;
		wallet->insert_adhoc (key.prv);
		guis.push_back (std::unique_ptr<galileo_qt::wallet> (new galileo_qt::wallet (application, processor, *system.nodes[i], wallet, key.pub)));
