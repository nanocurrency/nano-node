...
#
# find_package(Boost [version] [EXACT]      # Minimum or EXACT version e.g.
# 1.67.0 [REQUIRED]             # Fail with error if Boost is not found
# [COMPONENTS <libs>...] # Boost libraries by their canonical name # e.g.
# "date_time" for "libboost_date_time" [OPTIONAL_COMPONENTS <libs>...] #
# Optional Boost libraries by their canonical name) )                      #
# e.g. "date_time" for "libboost_date_time"
#
# This module finds headers and requested component libraries OR a CMake package
# configuration file provided by a "Boost CMake" build.  For the latter case
# skip to the "Boost CMake" section below.  For the former case results are
# reported in variables::
#
# Boost_FOUND            - True if headers and requested libraries were found
# Boost_INCLUDE_DIRS     - Boost include directories Boost_LIBRARY_DIRS     -
# Link directories for Boost libraries Boost_LIBRARIES        - Boost component
# libraries to be linked Boost_<C>_FOUND        - True if component <C> was
# found (<C> is upper-case) Boost_<C>_LIBRARY      - Libraries to link for
# component <C> (may include target_link_libraries debug/optimized keywords)
# Boost_VERSION          - BOOST_VERSION value from boost/version.hpp
# Boost_LIB_VERSION      - Version string appended to library filenames
# Boost_MAJOR_VERSION    - Boost major version number (X in X.y.z)
# Boost_MINOR_VERSION    - Boost minor version number (Y in x.Y.z)
# Boost_SUBMINOR_VERSION - Boost subminor version number (Z in x.y.Z)
# Boost_LIB_DIAGNOSTIC_DEFINITIONS (Windows) - Pass to add_definitions() to have
# diagnostic information about Boost's automatic linking displayed during
# compilation
#
# Note that Boost Python components require a Python version suffix (Boost 1.67
# and later), e.g. ``python36`` or ``python27`` for the versions built against
# Python 3.6 and 2.7, respectively.  This also applies to additional components
# using Python including ``mpi_python`` and ``numpy``.  Earlier Boost releases
# may use distribution-specific suffixes such as ``2``, ``3`` or ``2.7``. These
...
# include directory) Boost::<C>                    - Target for specific
# component dependency (shared or static library); <C> is lower- case
# Boost::diagnostic_definitions - interface target to enable diagnostic
# information about Boost's automatic linking during compilation (adds
# BOOST_LIB_DIAGNOSTIC) Boost::disable_autolinking    - interface target to
# disable automatic linking with MSVC (adds BOOST_ALL_NO_LIB)
# Boost::dynamic_linking        - interface target to enable dynamic linking
# linking with MSVC (adds BOOST_ALL_DYN_LINK)
#
# Implicit dependencies such as Boost::filesystem requiring Boost::system will
# be automatically detected and satisfied, even if system is not specified when
# using find_package and if Boost::system is not added to target_link_libraries.
# If using Boost::thread, then Threads::Threads will also be added
# automatically.
#
# It is important to note that the imported targets behave differently than
# variables created by this module: multiple calls to find_package(Boost) in the
# same directory or sub-directories with different options (e.g. static or
...
# read these entries directly but instead use the above result variables.  Note
# that some hint names start in upper-case "BOOST".  One may specify these as
# environment variables if they are not specified as CMake variables or cache
# entries.
#
...
# settings below.  It saves the library directories in Boost_LIBRARY_DIR_DEBUG
...
#
# Boost_USE_DEBUG_LIBS     - Set to ON or OFF to specify whether to search and
# use the debug libraries.  Default is ON. Boost_USE_RELEASE_LIBS   - Set to ON
# or OFF to specify whether to search and use the release libraries.  Default is
# ON. Boost_USE_MULTITHREADED  - Set to OFF to use the non-multithreaded
# libraries ('mt' tag).  Default is ON. Boost_USE_STATIC_LIBS    - Set to ON to
# force the use of the static libraries.  Default is OFF.
# Boost_USE_STATIC_RUNTIME - Set to ON or OFF to specify whether to use
# libraries linked statically to the C++ runtime ('s' tag).  Default is platform
# dependent. Boost_USE_DEBUG_RUNTIME  - Set to ON or OFF to specify whether to
# use libraries linked to the MS debug C++ runtime ('g' tag).  Default is ON.
# Boost_USE_DEBUG_PYTHON   - Set to ON to use libraries compiled with a debug
# Python build ('y' tag). Default is OFF. Boost_USE_STLPORT        - Set to ON
# to use libraries compiled with STLPort ('p' tag).  Default is OFF.
# Boost_USE_STLPORT_DEPRECATED_NATIVE_IOSTREAMS - Set to ON to use libraries
# compiled with STLPort deprecated "native iostreams" ('n' tag).  Default is
# OFF. Boost_COMPILER           - Set to the compiler-specific library suffix
# (e.g. "-gcc43").  Default is auto-computed for the C++ compiler in use.  A
# list may be used if multiple compatible suffixes should be tested for, in
# decreasing order of preference. Boost_ARCHITECTURE       - Set to the
# architecture-specific library suffix (e.g. "-x64").  Default is auto-computed
# for the C++ compiler in use. Boost_THREADAPI          - Suffix for "thread"
# component library name, such as "pthread" or "win32".  Names with and without
# this suffix will both be tried. Boost_NAMESPACE          - Alternate namespace
# used to build boost with e.g. if set to "myboost", will search for
# myboost_thread instead of boost_thread.
#
# Other variables one may set to control this module are::
#
# Boost_DEBUG              - Set to ON to enable debug output from FindBoost.
# Please enable this before filing any bug report. Boost_DETAILED_FAILURE_MSG -
# Set to ON to add detailed information to the failure message even when the
# REQUIRED option is not given to the find_package call. Boost_REALPATH - Set to
# ON to resolve symlinks for discovered libraries to assist with packaging.  For
# example, the "system" component library may be resolved to
# "/usr/lib/libboost_system.so.1.67.0" instead of "/usr/lib/libboost_system.so".
# This does not affect linking and should not be enabled unless the user needs
# this information. Boost_LIBRARY_DIR        - Default value for
# Boost_LIBRARY_DIR_RELEASE and Boost_LIBRARY_DIR_DEBUG.
#
# On Visual Studio and Borland compilers Boost headers request automatic linking
# to corresponding libraries.  This requires matching libraries to be linked
# explicitly or available in the link library search path. In this case setting
...
# include_directories(${Boost_INCLUDE_DIRS}) add_executable(foo foo.cc) endif()
#
# Example to find Boost libraries and use imported targets::
#
# find_package(Boost 1.56 REQUIRED COMPONENTS date_time filesystem iostreams)
# add_executable(foo foo.cc) target_link_libraries(foo Boost::date_time
# Boost::filesystem Boost::iostreams)
#
# Example to find Boost Python 3.6 libraries and use imported targets::
#
# find_package(Boost 1.67 REQUIRED COMPONENTS python36 numpy36)
# add_executable(foo foo.cc) target_link_libraries(foo Boost::python36
# Boost::numpy36)
#
...
# if(Boost_FOUND) include_directories(${Boost_INCLUDE_DIRS}) add_executable(foo
# foo.cc) target_link_libraries(foo ${Boost_LIBRARIES}) endif()
#
# Boost CMake
# ^^^^^^^^^^^
#
# If Boost was built using the boost-cmake project it provides a package
# configuration file for use with find_package's Config mode.  This module looks
# for the package configuration file called BoostConfig.cmake or
...
# no further action.  See documentation of the Boost CMake package configuration
...
#
# If Boost_DIR is set, this behaves as any find_package call would. If not, it
# looks at BOOST_ROOT and BOOSTROOT to find Boost.
#
...
# original authors of the FindQt4.cmake file. Only minor modifications were made
# to remove references to Qt and make this file more generally applicable And
# ELSE/ENDIF pairs were removed for readability.
# ##############################################################################
...
macro(_Boost_ADJUST_LIB_VARS basename)
  if(Boost_INCLUDE_DIR)
    ...
    if(Boost_${basename}_LIBRARY AND Boost_${basename}_HEADER)
      ...
      if("x${basename}" STREQUAL "xTHREAD" AND NOT TARGET Threads::Threads)
        string(APPEND Boost_ERROR_REASON_THREAD
  ...
  mark_as_advanced(Boost_${basename}_LIBRARY_RELEASE
                   Boost_${basename}_LIBRARY_DEBUG)
endmacro()

# Detect changes in used variables. Compares the current variable value with the
# last one. In short form: v != v_LAST                      -> CHANGED = 1 v is
# defined, v_LAST not         -> CHANGED = 1 v is not defined, but v_LAST is  ->
# CHANGED = 1 otherwise                        -> CHANGED = 0 CHANGED is
# returned in variable named ${changed_var}
...
#
# Find the given library (var). Use 'build_type' to support different lib paths
# for RELEASE or DEBUG builds
#
macro(_Boost_FIND_LIBRARY var build_type)
  ...
  elseif(_Boost_FIND_LIBRARY_HINTS_FOR_COMPONENT)
    # Try component-specific hints but do not save
    # Boost_LIBRARY_DIR_[RELEASE,DEBUG].
  ...
  if(Boost_LIBRARY_DIR_${build_type})
    set(_boost_LIBRARY_SEARCH_DIRS_${build_type}
        ${Boost_LIBRARY_DIR_${build_type}} NO_DEFAULT_PATH
...
#
# Runs compiler with "-dumpversion" and parses major/minor version with a regex.
#
...
# Guesses Boost's compiler prefix used in built library names Returns the guess
# by setting the variable pointed to by _ret
function(_Boost_GUESS_COMPILER_PREFIX _ret)
  ...
  elseif(UNIX)
    ...
    if(NOT Boost_VERSION VERSION_LESS 106900)
      ...
      # those compilers, patch is exclude from compiler tag in Boost 1.70+
      # library naming.
        ...
        if(APPLE)
          if(Boost_MINOR_VERSION)
            if(${Boost_MINOR_VERSION} GREATER 35)
              ...
              # In Boost <= 1.35.0, there is no mangled compiler name for the
              # macOS/Darwin version of GCC.
              set(_boost_COMPILER "")
            endif()
          else()
...
#
# Get component dependencies.  Requires the dependencies to have been defined
# for the Boost release version.
#
...
function(_Boost_COMPONENT_DEPENDENCIES component _ret)
  ...
  #
  # % cmake -DBOOST_DIR=/path/to/boost/source -P
  # Utilities/Scripts/BoostScanDeps.cmake
  #
  # The output may be added in a new block below.  If it's the same as the
  # previous release, simply update the version range of the block for the
  # previous release.  Also check if any new components have been added, and add
  # any new components to _Boost_COMPONENT_HEADERS.
  #
  # This information was originally generated by running BoostScanDeps.cmake
  # against every boost release to date supported by FindBoost:
  #
  ...
  # * Indent s;^set(;    set(;;
  # * Add conditionals s;Scanning
  #   /path/to/boost/sources/boost_\(.*\)_\(.*\)_\(.*);  elseif(NOT
  #   Boost_VERSION VERSION_LESS \10\20\3 AND Boost_VERSION VERSION_LESS xxxx);
  #
  # This results in the logic seen below, but will require the xxxx replacing
  # with the following Boost release version (or the next minor version to be
  # released, e.g. 1.59 was the latest at the time of writing, making 1.60 the
  # next, so 106000 is the needed version number).  Identical consecutive
  # releases were then merged together by updating the end range of the first
  # block and removing the following redundant blocks.
  #
  # Running the script against all historical releases should be required only
  # if the BoostScanDeps.cmake script logic is changed. The addition of a new
  # release should only require it to be run against the new release.

  # Handle Python version suffixes
  if(component MATCHES
     "^(python|mpi_python|numpy)([0-9][0-9]?|[0-9]\\.[0-9])\$")
  ...
  elseif(NOT Boost_VERSION VERSION_LESS 103300 AND Boost_VERSION VERSION_LESS
                                                   103500)
  ...
  elseif(NOT Boost_VERSION VERSION_LESS 103500 AND Boost_VERSION VERSION_LESS
                                                   103600)
    ...
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 103600 AND Boost_VERSION VERSION_LESS
                                                   103800)
    set(_Boost_FILESYSTEM_DEPENDENCIES system)
    set(_Boost_IOSTREAMS_DEPENDENCIES regex)
    ...
    set(_Boost_WAVE_DEPENDENCIES filesystem system thread)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 103800 AND Boost_VERSION VERSION_LESS
                                                   104300)
    set(_Boost_FILESYSTEM_DEPENDENCIES system)
    set(_Boost_IOSTREAMS_DEPENDENCIES regex)
    set(_Boost_MATH_DEPENDENCIES math_c99 math_c99f math_c99l math_tr1
    ...
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 104300 AND Boost_VERSION VERSION_LESS
                                                   104400)
    set(_Boost_FILESYSTEM_DEPENDENCIES system)
    set(_Boost_IOSTREAMS_DEPENDENCIES regex)
    ...
    set(_Boost_WAVE_DEPENDENCIES filesystem system thread date_time)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 104400 AND Boost_VERSION VERSION_LESS
                                                   104500)
    set(_Boost_FILESYSTEM_DEPENDENCIES system)
    set(_Boost_IOSTREAMS_DEPENDENCIES regex)
    set(_Boost_MATH_DEPENDENCIES
    ...
    set(_Boost_MPI_PYTHON_DEPENDENCIES python${component_python_version} mpi
                                       serialization)
    set(_Boost_THREAD_DEPENDENCIES date_time)
    set(_Boost_WAVE_DEPENDENCIES serialization filesystem system thread
                                 date_time)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 104500 AND Boost_VERSION VERSION_LESS
                                                   104700)
    set(_Boost_FILESYSTEM_DEPENDENCIES system)
    set(_Boost_IOSTREAMS_DEPENDENCIES regex)
    set(_Boost_MATH_DEPENDENCIES
    ...
    set(_Boost_MPI_PYTHON_DEPENDENCIES python${component_python_version} mpi
                                       serialization)
    set(_Boost_THREAD_DEPENDENCIES date_time)
    set(_Boost_WAVE_DEPENDENCIES filesystem system serialization thread
                                 date_time)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 104700 AND Boost_VERSION VERSION_LESS
                                                   104800)
    ...
    set(_Boost_WAVE_DEPENDENCIES filesystem system serialization thread
                                 date_time)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 104800 AND Boost_VERSION VERSION_LESS
                                                   105000)
    set(_Boost_CHRONO_DEPENDENCIES system)
    set(_Boost_FILESYSTEM_DEPENDENCIES system)
    set(_Boost_IOSTREAMS_DEPENDENCIES regex)
    ...
    set(_Boost_WAVE_DEPENDENCIES filesystem system serialization thread
                                 date_time)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 105000 AND Boost_VERSION VERSION_LESS
                                                   105300)
    set(_Boost_CHRONO_DEPENDENCIES system)
    set(_Boost_FILESYSTEM_DEPENDENCIES system)
    set(_Boost_IOSTREAMS_DEPENDENCIES regex)
    ...
    set(_Boost_WAVE_DEPENDENCIES filesystem system serialization thread chrono
                                 date_time)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 105300 AND Boost_VERSION VERSION_LESS
                                                   105400)
    ...
    set(_Boost_WAVE_DEPENDENCIES filesystem system serialization thread chrono
                                 date_time)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 105400 AND Boost_VERSION VERSION_LESS
                                                   105500)
    set(_Boost_ATOMIC_DEPENDENCIES thread chrono system date_time)
    set(_Boost_CHRONO_DEPENDENCIES system)
    set(_Boost_FILESYSTEM_DEPENDENCIES system)
    ...
    set(_Boost_WAVE_DEPENDENCIES
        ...
        atomic)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 105500 AND Boost_VERSION VERSION_LESS
                                                   105600)
    set(_Boost_CHRONO_DEPENDENCIES system)
    ...
    set(_Boost_WAVE_DEPENDENCIES
        ...
        atomic)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 105600 AND Boost_VERSION VERSION_LESS
                                                   105900)
    set(_Boost_CHRONO_DEPENDENCIES system)
    set(_Boost_COROUTINE_DEPENDENCIES context system)
    set(_Boost_FILESYSTEM_DEPENDENCIES system)
    ...
    set(_Boost_WAVE_DEPENDENCIES
        ...
        atomic)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 105900 AND Boost_VERSION VERSION_LESS
                                                   106000)
    set(_Boost_CHRONO_DEPENDENCIES system)
    set(_Boost_COROUTINE_DEPENDENCIES context system)
    set(_Boost_FILESYSTEM_DEPENDENCIES system)
  ...
  elseif(NOT Boost_VERSION VERSION_LESS 106300 AND Boost_VERSION VERSION_LESS
    ...
    set(_Boost_WAVE_DEPENDENCIES
        ...
        atomic)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 106500 AND Boost_VERSION VERSION_LESS
                                                   106700)
    set(_Boost_CHRONO_DEPENDENCIES system)
    set(_Boost_CONTEXT_DEPENDENCIES thread chrono system date_time)
    set(_Boost_COROUTINE_DEPENDENCIES context system)
    ...
    set(_Boost_WAVE_DEPENDENCIES
        ...
        atomic)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 106700 AND Boost_VERSION VERSION_LESS
                                                   106800)
    set(_Boost_CHRONO_DEPENDENCIES system)
    set(_Boost_CONTEXT_DEPENDENCIES thread chrono system date_time)
    set(_Boost_COROUTINE_DEPENDENCIES context system)
    ...
    set(_Boost_WAVE_DEPENDENCIES
        ...
        atomic)
    set(_Boost_WSERIALIZATION_DEPENDENCIES serialization)
  elseif(NOT Boost_VERSION VERSION_LESS 106800 AND Boost_VERSION VERSION_LESS
                                                   106900)
    set(_Boost_CHRONO_DEPENDENCIES system)
    set(_Boost_CONTEXT_DEPENDENCIES thread chrono system date_time)
  ...
  # message(STATUS "Dependencies for Boost::${component}:
  # ${_boost_DEPS_STRING}")
endfunction()

#
# Get component headers.  This is the primary header (or headers) for a given
# component, and is used to check that the headers are present as well as the
# library itself as an extra sanity check of the build environment.
#
...
function(_Boost_COMPONENT_HEADERS component _hdrs)
  ...
  # Note: new boost components will require adding here.  The header must be
  # present in all versions of Boost providing a library.
  ...
  string(REGEX REPLACE ";" " " _boost_HDRS_STRING
                       "${_Boost_${uppercomponent}_HEADERS}")
...
# _Boost_IMPORTED_TARGETS (TRUE if imported targets should be defined; FALSE if
# dependency information is unavailable).
#
# componentvar - the component list variable name extravar - the indirect
# dependency list variable name
#
function(_Boost_MISSING_DEPENDENCIES componentvar extravar)
  ...
  # being processed) _boost_new_components - new components discovered for
  # future processing
  #
...
#
# Some boost libraries may require particular set of compler features. The very
# first one was `boost::fiber` introduced in Boost 1.62. One can check required
# compiler features of it in `${Boost_ROOT}/libs/fiber/build/Jamfile.v2`.
#
...
#
# Update library search directory hint variable with paths used by prebuilt
# boost binaries.
#
# Prebuilt windows binaries
# (https://sourceforge.net/projects/boost/files/boost-binaries/) have library
# directories named using MSVC compiler version and architecture. This function
# would append corresponding directories if MSVC is a current compiler, so
# having `BOOST_ROOT` would be enough to specify to find everything.
#
function(_Boost_UPDATE_WINDOWS_LIBRARY_SEARCH_DIRS_WITH_PREBUILT_PATHS
...
if(Boost_FIND_VERSION_EXACT)
  # The version may appear in a directory with or without the patch level, even
  # when the patch level is non-zero.
  ...
  # information in _Boost_COMPONENT_DEPENDENCIES and _Boost_COMPONENT_HEADERS.
  # See the instructions at the top of _Boost_COMPONENT_DEPENDENCIES.
...
# The reason that we failed to find Boost. This will be set to a user-friendly
# message when we fail to find some necessary piece of Boost.
...
# Supply Boost_LIB_DIAGNOSTIC_DEFINITIONS as a convenience target. It will only
# contain any interface definitions on WIN32, but is created on all platforms to
# keep end user code free from platform dependent code.  Also provide
# convenience targets to disable autolinking and enable dynamic linking.
...
if(WIN32)
  ...
  # the libraries.  If you are linking to a dynamic runtime, then you can choose
  # to link to either a static or a dynamic Boost library, the default is to do
  # a static link.  You can alter this for a specific library "whatever" by
  # defining BOOST_WHATEVER_DYN_LINK to force Boost library "whatever" to be
  # linked dynamically.  Alternatively you can force all Boost libraries to
  # dynamic link by defining BOOST_ALL_DYN_LINK.
  ...
  set_target_properties(
    Boost::diagnostic_definitions PROPERTIES INTERFACE_COMPILE_DEFINITIONS
                                             "BOOST_LIB_DIAGNOSTIC")
  set_target_properties(
    Boost::disable_autolinking PROPERTIES INTERFACE_COMPILE_DEFINITIONS
                                          "BOOST_ALL_NO_LIB")
  set_target_properties(
    Boost::dynamic_linking PROPERTIES INTERFACE_COMPILE_DEFINITIONS
                                      "BOOST_ALL_DYN_LINK")
endif()
...
set(_Boost_VARS_INC BOOST_INCLUDEDIR Boost_INCLUDE_DIR
                    Boost_ADDITIONAL_VERSIONS)
...
if(NOT Boost_INCLUDE_DIR)
  ...
  # Try to find Boost by stepping backwards through the Boost versions we know
  # about. Build a list of path suffixes for each version.
  set(_boost_PATH_SUFFIXES)
  foreach(_boost_VER ${_boost_TEST_VERSIONS})
    # Add in a path suffix, based on the required version, ideally we could read
    # this from version.hpp, but for that to work we'd need to know the include
    # dir already
...
# Set Boost_FOUND based only on header location and version. It will be updated
# below for component libraries.
if(Boost_INCLUDE_DIR)
  if(Boost_DEBUG)
    message(
      STATUS "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] "
  ...
  file(STRINGS "${Boost_INCLUDE_DIR}/boost/version.hpp"
       _boost_VERSION_HPP_CONTENTS REGEX "#define BOOST_(LIB_)?VERSION ")
...
if(Boost_COMPILER)
  ...
  # experience any issues please report them and use the Boost_COMPILER variable
...
# ======================
# Systematically build up the Boost ABI tag for the 'tagged' and 'versioned'
# layouts
...
# Key       Use this library when: s        linking statically to the C++
# standard library and compiler runtime support libraries.
...
# n        using the STLport deprecated "native iostreams" feature removed from
# the documentation in 1.43.0 but still present in boost/config/auto_link.hpp
if(Boost_USE_STLPORT_DEPRECATED_NATIVE_IOSTREAMS)
  string(APPEND _boost_RELEASE_ABI_TAG "n")
  string(APPEND _boost_DEBUG_ABI_TAG "n")
endif()

# -x86     Architecture and address model tag First character is the
# architecture, then word-size, either 32 or 64 Only used in 'versioned' layout,
# added in Boost 1.66.0
if(DEFINED Boost_ARCHITECTURE)
  ...
  if(NOT "x${CMAKE_CXX_COMPILER_ARCHITECTURE_ID}" STREQUAL "x"
     AND NOT Boost_VERSION VERSION_LESS 106600)
...
foreach(c DEBUG RELEASE)
  ...
  # Clear Boost_LIBRARY_DIR_${c} if it did not change but other input affecting
  ...
  if(Boost_LIBRARY_DIR_${c})
    set(_boost_LIBRARY_SEARCH_DIRS_${c} ${Boost_LIBRARY_DIR_${c}}
...
# The previous behavior of FindBoost when Boost_USE_STATIC_LIBS was enabled on
# WIN32 was to: 1. Search for static libs compiled against a SHARED C++ standard
# runtime library (use if found) 2. Search for static libs compiled against a
# STATIC C++ standard runtime library (use if found) We maintain this behavior
# since changing it could break people's builds. To disable the ambiguous
# behavior, the user need only set Boost_USE_STATIC_RUNTIME either ON or OFF.
...
# On versions < 1.35, remove the System library from the considered list since
# it wasn't added until 1.35.
...
# Additional components may be required via component dependencies. Add any
# missing components to the list.
...
foreach(COMPONENT ${Boost_FIND_COMPONENTS})
  ...
  foreach(component IN LISTS _Boost_FIND_LIBRARY_HINTS_FOR_COMPONENT_NAME
                             COMPONENT)
  ...
  # if Boost_LIBRARY_DIR_RELEASE is not defined, but Boost_LIBRARY_DIR_DEBUG is,
  # look there first for RELEASE libs
  ...
  # if Boost_LIBRARY_DIR_DEBUG is not defined, but Boost_LIBRARY_DIR_RELEASE is,
  # look there first for DEBUG libs
  ...
  _boost_compiler_features(${COMPONENT}
                           _Boost_${UPPERCOMPONENT}_COMPILER_FEATURES)
...
# The above setting of Boost_FOUND was based only on the header files. Update it
# for the requested component libraries.
if(Boost_FOUND)
  ...
  if(_Boost_MISSING_COMPONENTS)
    set(Boost_FOUND 0)
    # We were unable to find some libraries, so generate a sensible error
    # message that lists the libraries we were unable to find.
  ...
  if(NOT Boost_LIBRARY_DIRS AND NOT _boost_CHECKED_COMPONENT)
    ...
    # Look for the boost library path. Note that the user may not have installed
    # any libraries so it is quite possible the Boost_LIBRARY_DIRS may not
    # exist.
...
if(Boost_FOUND)
  ...
  if(NOT TARGET Boost::boost)
    ...
    if(Boost_INCLUDE_DIRS)
      set_target_properties(
        Boost::boost PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                "${Boost_INCLUDE_DIRS}")
    endif()
  endif()
...
if(Boost_FOUND)
  ...
  if(Boost_FIND_REQUIRED)
    ...
    if(NOT Boost_FIND_QUIETLY)
      # we opt not to automatically output Boost_ERROR_REASON here as it could
      # be quite lengthy and somewhat imposing in its requests Since Boost is
      # not always a required dependency we'll leave this up to the end-user.
...
# Record last used values of input variables so we can detect on the next run if
# the user changed them.
foreach(v ${_Boost_VARS_INC} ${_Boost_VARS_LIB} ${_Boost_VARS_DIR}
          ${_Boost_VARS_NAME})
  if(DEFINED ${v})
    set(_${v}_LAST
        "${${v}}"
...
# Maintain a persistent list of components requested anywhere since the last
# flush.
