...
rai::musig_stage1_req::musig_stage1_req (rai::uint256_union req_id_a, rai::uint256_union rb_total_a, rai::public_key agg_pubkey_a, rai::uint256_union l_base_a, rai::keypair keypair_a) :
...
rai::uint256_union rai::musig_stage1_req::hash () const
{
	...
	assert (status == 0);
	status = blake2b_update (&hash_l, l_base.bytes.data (), sizeof (l_base));
	assert (status == 0);
	status = blake2b_final (&hash_l, result.bytes.data (), sizeof (result.bytes));
	assert (status == 0);
	return result;
...
bool rai::musig_stage1_req::deserialize (rai::stream & stream_a)
{
	...
	if (!result)
	{
		...
		if (!result)
		{
			...
			if (!result)
			{
				result = rai::read (stream_a, l_base);
				if (!result)
				{
					result = rai::read (stream_a, node_id_signature);
				}
...
void rai::musig_stage1_req::serialize (rai::stream & stream_a)
{
	...
	rai::write (stream_a, l_base);
	rai::write (stream_a, node_id_signature);
}

bool rai::musig_stage1_req::operator== (rai::musig_stage1_req const & other_a) const
{
	auto result (request_id == other_a.request_id && rb_total == other_a.rb_total && agg_pubkey == other_a.agg_pubkey && l_base == other_a.l_base && node_id_signature == other_a.node_id_signature);
	return result;
}
