...
rai::process_return rai::block_processor::process_receive_one (rai::transaction const & transaction_a, std::shared_ptr<rai::block> block_a, std::chrono::steady_clock::time_point origination, rai::signature_verification verification)
{
	...
	switch (result.code)
	{
		...
		case rai::process_result::gap_previous:
		{
			...
			node.store.unchecked_put (transaction_a, rai::unchecked_key (block_a->previous (), block_a->hash ()), rai::unchecked_info (block_a, rai::seconds_since_epoch (), result.verified));
		...
		case rai::process_result::gap_source:
		{
			...
			node.store.unchecked_put (transaction_a, rai::unchecked_key (node.ledger.block_source (transaction_a, *block_a), block_a->hash ()), rai::unchecked_info (block_a, rai::seconds_since_epoch (), result.verified));
			node.gap_cache.add (transaction_a, block_a);
			break;
		}
