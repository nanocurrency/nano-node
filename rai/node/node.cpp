...
block_processor_thread ([this]() { this->block_processor.process_blocks (); })
{
	...
	{
		auto max_blocks = (uint64_t)bootstrap_weights.block_height.number ();
		rai::transaction transaction (store.environment, nullptr, false);
		if (ledger.store.block_count (transaction).sum () < max_blocks)
		{
			ledger.bootstrap_weight_max_blocks = max_blocks;
			for (auto & account_weight : bootstrap_weights.weights)
			{
...
rai::bootstrap_rep_weights::bootstrap_rep_weights ()
{
	...
	if (!rai::read (weight_stream, block_height))
	{
		...
		while (!rai::read (weight_stream, account.bytes) && !rai::read (weight_stream, weight.bytes))
		{
			weights.push_back (std::make_pair (account, weight));
		}
	}
}
