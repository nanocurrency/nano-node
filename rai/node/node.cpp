...
void rai::network::confirm_hashes (rai::transaction const & transaction_a, rai::endpoint const & peer_a, std::vector<rai::block_hash> blocks_bundle_a)
{
...
void rai::network::republish_block (std::shared_ptr<rai::block> block, rai::endpoint const & peer_a)
{
	auto hash (block->hash ());
	rai::publish message (block);
	std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
