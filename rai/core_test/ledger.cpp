...
TEST (votes, hash2)
{
	bool init (false);
	rai::block_store store (init, rai::unique_path ());
	auto receive (std::make_shared <rai::receive_block> (0, 0, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0));
	auto change (std::make_shared <rai::change_block> (0, 0, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0));
	rai::block_hash receive_hash2;
	rai::block_hash change_hash2;
	rai::transaction transaction (store.environment, nullptr, true);
	store.hash2_put (transaction, 0, 1);
	receive_hash2 = store.hash2_calc (transaction, *receive);
	change_hash2 = store.hash2_calc (transaction, *change);
	ASSERT_FALSE (receive_hash2.is_zero ());
	ASSERT_FALSE (change_hash2.is_zero ());
	auto vote1 (std::make_shared <rai::vote> (rai::test_genesis_key.pub, rai::test_genesis_key.prv, 1, change));
	auto vote2 (std::make_shared <rai::vote> (rai::test_genesis_key.pub, rai::test_genesis_key.prv, 2, receive, receive_hash2));
	ASSERT_EQ (rai::vote_code::vote, store.vote_validate (transaction, vote1).code);
	ASSERT_EQ (rai::vote_code::vote2, store.vote_validate (transaction, vote2).code);
}
