...
TEST (block_store, receivable)
{
    bool init (false);
    rai::block_store store (init, rai::unique_path ());
	ASSERT_TRUE (!init);
	rai::transaction transaction (store.environment, nullptr, true);
	rai::receivable_info receivable1 (1, 2);
	ASSERT_EQ (store.receivable_end (), store.receivable_begin (transaction));
	store.receivable_put (transaction, receivable1);
	auto current1 (store.receivable_begin (transaction));
	ASSERT_NE (store.receivable_end(), current1);
	rai::receivable_info receivable2 (current1->first);
	ASSERT_EQ (receivable1.account, receivable2.account);
	ASSERT_EQ (receivable1.hash, receivable2.hash);
	++current1;
	ASSERT_EQ (store.receivable_end (), current1);
}

TEST (block_store, receivable_find)
{
    bool init (false);
    rai::block_store store (init, rai::unique_path ());
	ASSERT_TRUE (!init);
	rai::transaction transaction (store.environment, nullptr, true);
	rai::receivable_info receivable1 (1, 3);
	store.receivable_put (transaction, receivable1);
	rai::receivable_info receivable2 (2, 4);
	store.receivable_put (transaction, receivable2);
	auto current1 (store.receivable_begin (transaction, 2));
	ASSERT_NE (store.receivable_end(), current1);
	rai::receivable_info receivable3 (current1->first);
	ASSERT_EQ (receivable2.account, receivable3.account);
	ASSERT_EQ (receivable2.hash, receivable3.hash);
}

TEST (block_store, v2_v3_upgrade)
{
	auto path1 (rai::unique_path ());
	{
		bool init (false);
		rai::block_store store (init, path1.string ());
		ASSERT_TRUE (!init);
		rai::transaction transaction (store.environment, nullptr, true);
		mdb_drop (transaction, store.receivable, 1);
		store.version_put (transaction, 2);
		rai::pending_info pending (1, 2, 3);
		store.pending_put (transaction, 4, pending);
	}
	{
		bool init (false);
		rai::block_store store (init, path1.string ());
		...
		auto current1 (store.receivable_begin (transaction, 3));
		ASSERT_NE (store.receivable_end (), current1);
		rai::receivable_info receivable1 (current1->first);
		ASSERT_EQ (rai::uint256_union (3), receivable1.account);
		ASSERT_EQ (rai::uint256_union (4), receivable1.hash);
	}
}