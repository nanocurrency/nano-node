...
TEST (network, multi_keepalive)
{
    ...
    rai::node_init init2;
    auto node2 (std::make_shared <rai::node> (init2, system.service, 24002, rai::unique_path (), system.processor, system.logging, system.work));
...
TEST (receivable_processor, send_with_receive)
{
    ...
    rai::block_hash latest1 (system.nodes [0]->latest (rai::test_genesis_key.pub));
	system.wallet (1)->insert (key2.prv);
    auto block1 (new rai::send_block (latest1, key2.pub, amount - 100, rai::test_genesis_key.prv, rai::test_genesis_key.pub, system.work.generate (latest1)));
...
TEST (network, receive_weight_change)
{
    ...
    ASSERT_FALSE (system.wallet (0)->send_sync (rai::test_genesis_key.pub, key2.pub, 2));
	auto iterations (0);
...
TEST (bootstrap_processor, process_one)
{
	rai::system system (24000, 1);
	system.wallet (0)->insert (rai::test_genesis_key.prv);
	ASSERT_FALSE (system.wallet (0)->send_sync (rai::test_genesis_key.pub, rai::test_genesis_key.pub, 100));
	rai::node_init init1;
	auto node1 (std::make_shared <rai::node> (init1, system.service, 24001, rai::unique_path (), system.processor, system.logging, system.work));
...
TEST (bootstrap_processor, process_two)
{
	...
	system.wallet (0)->insert (rai::test_genesis_key.prv);
	rai::block_hash hash1 (system.nodes [0]->latest (rai::test_genesis_key.pub));
	ASSERT_FALSE (system.wallet (0)->send_sync (rai::test_genesis_key.pub, rai::test_genesis_key.pub, 50));
...
TEST (bootstrap_processor, process_new)
{
	...
	rai::node_init init1;
	auto node1 (std::make_shared <rai::node> (init1, system.service, 24002, rai::unique_path (), system.processor, system.logging, system.work));
	ASSERT_FALSE (init1.error ());
	node1->bootstrap_initiator.bootstrap (system.nodes [0]->network.endpoint ());
	auto iterations2 (0);
...
TEST (bootstrap_processor, push_one)
{
    ...
    ASSERT_FALSE (wallet->send_sync (rai::test_genesis_key.pub, key1.pub, 100));
