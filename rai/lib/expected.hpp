...
#pragma warning(push)
#define nsel_DISABLE_MSVC_WARNINGS(codes) __pragma (warning (disable \
	                                                         : codes))
#else
...
namespace nonstd
{
	...
	public:
		...
		template <typename E2, nsel_REQUIRES_T (std::is_constructible<E, E2 &&>::value)>
		...
		constexpr explicit unexpected_type (unexpected_type<E2> const & error, nsel_REQUIRES (std::is_constructible<E, E2 const &>::value && !std::is_convertible<E2 const &, E>::value /*=> explicit */)) :
		m_error (error)
		{
		}

		template <typename E2>
		constexpr /*non-explicit*/ unexpected_type (unexpected_type<E2> const & error, nsel_REQUIRES (std::is_constructible<E, E2 const &>::value && std::is_convertible<E2 const &, E>::value /*=> non-explicit */)) :
		m_error (error)
		{
		}

		template <typename E2>
		constexpr explicit unexpected_type (unexpected_type<E2> && error, nsel_REQUIRES (std::is_constructible<E, E2 &&>::value && !std::is_convertible<E2 &&, E>::value /*=> explicit */)) :
		m_error (error)
		{
		}

		template <typename E2>
		constexpr /*non-explicit*/ unexpected_type (unexpected_type<E2> && error, nsel_REQUIRES (std::is_constructible<E, E2 &&>::value && std::is_convertible<E2 &&, E>::value /*=> non-explicit */)) :
		m_error (error)
		{
		}
	...
	public:
		...
		has_value_ (rhs.has_value_)
		{
			if (has_value ())
				...
				contained.construct_error (rhs.contained.error ());
		}
		...
		has_value_ (rhs.has_value_)
		{
			if (has_value ())
				...
				contained.construct_error (std::move (rhs.contained.error ()));
		}
		...
		template <typename U, typename G>
		nsel_constexpr14 /*non-explicit*/ expected (expected<U, G> const & rhs, nsel_REQUIRES (std::is_constructible<T, const U &>::value && std::is_constructible<E, const G &>::value && !std::is_constructible<T, expected<U, G> &>::value && !std::is_constructible<T, expected<U, G> &&>::value && !std::is_constructible<T, const expected<U, G> &>::value && !std::is_constructible<T, const expected<U, G> &&>::value && !std::is_convertible<expected<U, G> &, T>::value && !std::is_convertible<expected<U, G> &&, T>::value && !std::is_convertible<const expected<U, G> &, T>::value && !std::is_convertible<const expected<U, G> &&, T>::value && !(!std::is_convertible<U const &, T>::value || !std::is_convertible<const G &, E>::value) /*=> explicit */)) :
		has_value_ (rhs.has_value_)
		{
			if (has_value ())
				contained.construct_value (rhs.contained.value ());
		...
		template <typename U = T>
		nsel_constexpr14 expected (U && v, nsel_REQUIRES (std::is_constructible<T, U &&>::value && !std::is_same<typename std20::remove_cvref<U>::type, in_place_t>::value && !std::is_same<expected<T, E>, typename std20::remove_cvref<U>::type>::value && !std::is_same<nonstd::unexpected_type<E>, typename std20::remove_cvref<U>::type>::value && std::is_convertible<U &&, T>::value /*=> non-explicit */
		                                   )) noexcept (std::is_nothrow_move_constructible<U>::value &&
		std::is_nothrow_move_constructible<E>::value) :
		has_value_ (true)
		{
		...
		template <typename U, typename... Args, nsel_REQUIRES_T (std::is_constructible<T, std::initializer_list<U>, Args &&...>::value)>
		...
		nsel_constexpr14 explicit expected (nonstd::unexpected_type<G> const & error, nsel_REQUIRES (!std::is_convertible<const G &, E>::value /*=> explicit */)) :
		has_value_ (false)
		{
		...
		template <typename G = E>
		nsel_constexpr14 /*non-explicit*/ expected (nonstd::unexpected_type<G> const & error, nsel_REQUIRES (std::is_convertible<const G &, E>::value /*=> non-explicit */)) :
		has_value_ (false)
		{
			contained.construct_error (error.value ());
		}
		...
		template <typename G = E>
		nsel_constexpr14 explicit expected (nonstd::unexpected_type<G> && error, nsel_REQUIRES (!std::is_convertible<G &&, E>::value /*=> explicit */)) :
		has_value_ (false)
		{
		...
		template <typename G = E>
		nsel_constexpr14 /*non-explicit*/ expected (nonstd::unexpected_type<G> && error, nsel_REQUIRES (std::is_convertible<G &&, E>::value /*=> non-explicit */)) :
		has_value_ (false)
		{
			contained.construct_error (std::move (error.value ()));
		}
		...
		template <typename U, nsel_REQUIRES_T (std::is_constructible<T, U>::value && std::is_assignable<T &, U>::value)>
		...
		std::is_nothrow_move_constructible<E>::value && std::is_nothrow_swappable<E &>::value
#else
		std::is_nothrow_move_constructible<T>::value && noexcept (std::swap (std::declval<T &> (), std::declval<T &> ())) && std::is_nothrow_move_constructible<E>::value && noexcept (std::swap (std::declval<E &> (), std::declval<E &> ()))
#endif
		)
		{
			using std::swap;
	...
	public:
		...
		)
		{
			...
			{
				contained.construct_error (std::move (rhs.error ()));
				swap (has_value_, rhs.has_value_);
			}
			else if (!bool(*this) && bool(rhs))
			{
