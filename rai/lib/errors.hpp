...
#define REGISTER_ERROR_CODES(enum_type)                                                                         \
	namespace rai                                                                                               \
	{                                                                                                           \
		...
		{                                                                                                       \
			...
			return instance;                                                                                    \
                                                                                                                ...
                                                                                                                \
		inline std::error_code make_error_code (rai::enum_type err)                                             \
		{                                                                                                       \
			return { static_cast<int> (err), enum_type##_category () };                                         \
		}                                                                                                       \
                                                                                                                \
		inline auto unexpected_error (rai::enum_type err) -> decltype (make_unexpected (make_error_code (err))) \
		{                                                                                                       \
	...
	{                                                                                                           \
		template <>                                                                                             \
		struct is_error_code_enum<rai::enum_type> : std::true_type                                              \
		{                                                                                                       \
		};                                                                                                      \
	}

REGISTER_ERROR_CODES (error_common);
REGISTER_ERROR_CODES (error_blocks);
