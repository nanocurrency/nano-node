...
if [ -n "$DOCKER_PASSWORD" ]; then
    ...
    if [[ "$TRAVIS_BRANCH" == "master" || "$TRAVIS_BRANCH" == "docker_cache" ]] && [[ "$TRAVIS_JOB_NAME" != "other" ]]; then
        if [[ "%TRAVIS_JOB_NAME" != "base" ]]; then
            ci_image_name="nanocurrency/nano-env:$TRAVIS_JOB_NAME"
        else
            ci_image_name="nanocurrency/nano-base"
        fi
