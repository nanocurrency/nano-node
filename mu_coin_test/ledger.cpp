...
TEST (votes, add_existing)
{
    ...
    vote2.sequence = 2;
    vote2.block = send2.clone ();
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, vote2.hash (), vote2.signature);
    votes1->vote (vote2);
    ASSERT_EQ (2, votes1->rep_votes.size ());
    ASSERT_NE (votes1->rep_votes.end (), votes1->rep_votes.find (mu_coin::test_genesis_key.pub));
    ASSERT_EQ (send2, *votes1->rep_votes [mu_coin::test_genesis_key.pub].second);
    auto winner (votes1->winner ());
...
TEST (fork, multi_flip)
{
    ...
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, send3->hash (), send3->signature);
