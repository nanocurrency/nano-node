...
TEST (ledger, genesis_balance)
{
    ...
    auto balance (ledger.account_balance (mu_coin::genesis_address));
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max (), balance);
    mu_coin::frontier frontier;
    ASSERT_FALSE (store.latest_get (mu_coin::genesis_address, frontier));
...
TEST (ledger, process_send)
{
    ...
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, hash1, send.signature);
    ASSERT_EQ (mu_coin::process_result::progress, ledger.process (send));
    ASSERT_EQ (50, ledger.account_balance (mu_coin::test_genesis_key.pub));
    mu_coin::frontier frontier2;
    ASSERT_FALSE (store.latest_get (mu_coin::test_genesis_key.pub, frontier2));
    ...
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 50, ledger.account_balance (key2.pub));
    ASSERT_EQ (50, ledger.weight (mu_coin::test_genesis_key.pub));
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 50, ledger.weight (key2.pub));
    mu_coin::frontier frontier3;
    ASSERT_FALSE (store.latest_get (mu_coin::test_genesis_key.pub, frontier3));
    ...
    ASSERT_EQ (mu_coin::test_genesis_key.pub, sender1);
    ASSERT_EQ (key2.pub, destination1);
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 50, amount1.number ());
	ASSERT_EQ (0, ledger.account_balance (key2.pub));
	ASSERT_EQ (50, ledger.account_balance (mu_coin::test_genesis_key.pub));
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max (), ledger.weight (mu_coin::test_genesis_key.pub));
    ASSERT_EQ (0, ledger.weight (key2.pub));
    mu_coin::frontier frontier6;
	ASSERT_FALSE (ledger.store.latest_get (mu_coin::test_genesis_key.pub, frontier6));
	ASSERT_EQ (hash1, frontier6.hash);
	ledger.rollback (frontier6.hash);
    mu_coin::frontier frontier7;
	ASSERT_FALSE (ledger.store.latest_get (mu_coin::test_genesis_key.pub, frontier7));
	...
	ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max (), ledger.account_balance (mu_coin::test_genesis_key.pub));
}
...
TEST (ledger, process_receive)
{
    ...
    mu_coin::sign_message (key2.prv, key2.pub, hash2, open.signature);
    ASSERT_EQ (mu_coin::process_result::progress, ledger.process (open));
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 50, ledger.weight (key3.pub));
    ...
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, hash3, send2.signature);
	...
	ASSERT_EQ (25, ledger.account_balance (mu_coin::test_genesis_key.pub));
	ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 25, ledger.account_balance (key2.pub));
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 25, ledger.weight (key3.pub));
...
TEST (ledger, rollback_representation)
{
    ...
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, change1.hash (), change1.signature);
    ...
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, change2.hash (), change2.signature);
    ...
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 1, ledger.weight (key4.pub));
    ledger.rollback (receive1.hash ());
    ASSERT_EQ (50, ledger.weight (key3.pub));
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 50, ledger.weight (key4.pub));
    ledger.rollback (open.hash ());
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max (), ledger.weight (key3.pub));
    ASSERT_EQ (0, ledger.weight (key4.pub));
    ledger.rollback (change2.hash ());
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max (), ledger.weight (key5.pub));
    ASSERT_EQ (0, ledger.weight (key3.pub));
}
...
TEST (processor_service, bad_send_signature)
{
    ...
    send.hashables.previous = frontier1.hash;
    send.hashables.balance = 50;
    send.hashables.destination = mu_coin::test_genesis_key.pub;
    mu_coin::block_hash hash1 (send.hash ());
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, hash1, send.signature);
...
TEST (ledger, representative_genesis)
{
    ...
    mu_coin::genesis genesis;
    genesis.initialize (store);
    ASSERT_EQ (mu_coin::test_genesis_key.pub, ledger.representative (ledger.latest (mu_coin::test_genesis_key.pub)));
}
...
TEST (ledger, representative_change)
{
    ...
    ASSERT_EQ (0, ledger.weight (mu_coin::test_genesis_key.pub));
...
TEST (ledger, send_fork)
{
    ...
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, block2.hash (), block2.signature);
...
TEST (ledger, receive_fork)
{
    ...
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, block4.hash (), block4.signature);
...
TEST (ledger, checksum_single)
{
    ...
    block1.hashables.previous = ledger.latest (mu_coin::test_genesis_key.pub);
...
TEST (system, generate_send_existing)
{
    ...
    mu_coin::frontier frontier2;
    ASSERT_FALSE (system.clients [0]->store.latest_get (mu_coin::test_genesis_key.pub, frontier2));
    ASSERT_NE (frontier1.hash, frontier2.hash);
    while (system.clients [0]->ledger.account_balance (mu_coin::test_genesis_key.pub) == std::numeric_limits <mu_coin::uint256_t>::max ())
    {
...
TEST (system, generate_send_new)
{
    ...
    if (iterator2->first != mu_coin::test_genesis_key.pub)
    {
...
TEST (ledger, representation)
{
    ...
    mu_coin::genesis genesis;
    genesis.initialize (store);
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max (), store.representation_get (mu_coin::test_genesis_key.pub));
    mu_coin::keypair key2;
    mu_coin::send_block block1;
    ...
    mu_coin::sign_message (key2.prv, key2.pub, block2.hash (), block2.signature);
    ASSERT_EQ (mu_coin::process_result::progress, ledger.process (block2));
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 100, store.representation_get (mu_coin::test_genesis_key.pub));
    ...
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, block3.hash (), block3.signature);
    ASSERT_EQ (mu_coin::process_result::progress, ledger.process (block3));
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 100, store.representation_get (mu_coin::test_genesis_key.pub));
    ASSERT_EQ (0, store.representation_get (key2.pub));
    ASSERT_EQ (100, store.representation_get (key3.pub));
    ...
    ASSERT_EQ (mu_coin::process_result::progress, ledger.process (block4));
    ASSERT_EQ (std::numeric_limits <mu_coin::uint256_t>::max () - 200, store.representation_get (mu_coin::test_genesis_key.pub));
    ASSERT_EQ (0, store.representation_get (key2.pub));
...
TEST (ledger, double_open)
{
    ...
    open2.hashables.representative = mu_coin::test_genesis_key.pub;
...
TEST (votes, add_unsigned)
{
    ...
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, send1.hash (), send1.signature);
    ASSERT_EQ (mu_coin::process_result::progress, client1.ledger.process (send1));
...
TEST (votes, add_one)
{
    ...
    vote1.sequence = 1;
    vote1.block = send1.clone ();
    vote1.address = mu_coin::test_genesis_key.pub;
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, vote1.hash (), vote1.signature);
    votes1->vote (vote1);
    ASSERT_EQ (2, votes1->rep_votes.size ());
    auto existing1 (votes1->rep_votes.find (mu_coin::test_genesis_key.pub));
...
TEST (votes, add_two)
{
    ...
    ASSERT_NE (votes1->rep_votes.end (), votes1->rep_votes.find (mu_coin::test_genesis_key.pub));
    ASSERT_EQ (send1, *votes1->rep_votes [mu_coin::test_genesis_key.pub].second);
...
TEST (votes, add_existing)
{
    ...
    mu_coin::sign_message (mu_coin::test_genesis_key.prv, mu_coin::test_genesis_key.pub, send2.hash (), send2.signature);
    mu_coin::vote vote2;
    vote2.address = mu_coin::test_genesis_key.pub;
    vote2.sequence = 2;
    vote2.block = send2.clone ();
    ...
    ASSERT_EQ (send2, *votes1->rep_votes [system.test_genesis_address.pub].second);
    auto winner (votes1->winner ());
...
TEST (votes, add_old)
{
    ...
    ASSERT_EQ (2, votes1->rep_votes.size ());
    ASSERT_NE (votes1->rep_votes.end (), votes1->rep_votes.find (system.test_genesis_address.pub));
    ASSERT_EQ (send1, *votes1->rep_votes [system.test_genesis_address.pub].second);
    auto winner (votes1->winner ());
    ASSERT_EQ (send1, *winner.first);
}
...
TEST (conflicts, add_two)
{
    ...
    send2.hashables.destination = key2.pub;
    mu_coin::sign_message (system.test_genesis_address.prv, system.test_genesis_address.pub, send2.hash (), send2.signature);
    ASSERT_EQ (mu_coin::process_result::progress, client1.ledger.process (send2));
    client1.conflicts.start (send2, false);
...
TEST (fork, publish)
{
    std::weak_ptr <mu_coin::client> client0;
    {
        ...
        client1.processor.process_message (publish1, mu_coin::endpoint {}, true);
        ASSERT_EQ (0, client1.conflicts.roots.size ());
        client1.processor.process_message (publish2, mu_coin::endpoint {}, true);
        ASSERT_EQ (1, client1.conflicts.roots.size ());
        ...
        ASSERT_EQ (2, votes1->rep_votes.size ());
        auto existing1 (votes1->rep_votes.find (system.test_genesis_address.pub));
        ASSERT_NE (votes1->rep_votes.end (), existing1);
...
TEST (fork, keep)
{
    ...
    publish2.block = std::move (send2);
    client1.processor.process_message (publish1, mu_coin::endpoint {}, true);
	client2.processor.process_message (publish1, mu_coin::endpoint {}, true);
    ASSERT_EQ (0, client1.conflicts.roots.size ());
    ASSERT_EQ (0, client2.conflicts.roots.size ());
    client1.processor.process_message (publish2, mu_coin::endpoint {}, true);
	client2.processor.process_message (publish2, mu_coin::endpoint {}, true);
    ASSERT_EQ (1, client1.conflicts.roots.size ());
    ASSERT_EQ (1, client2.conflicts.roots.size ());
    auto conflict (client2.conflicts.roots.find (system.genesis.hash ()));
...
TEST (fork, multi_flip)
{
    ...
    mu_coin::publish_req publish2;
    publish2.block = std::move (send2);
    std::unique_ptr <mu_coin::send_block> send3 (new mu_coin::send_block);
    send3->hashables.previous = publish2.block->hash ();
    send3->hashables.balance.clear ();
    send3->hashables.destination = key2.pub;
    mu_coin::sign_message (system.test_genesis_address.prv, system.test_genesis_address.pub, send3->hash (), send3->signature);
    mu_coin::publish_req publish3;
    publish3.block = std::move (send3);
    client1.processor.process_message (publish1, mu_coin::endpoint {}, true);
	client2.processor.process_message (publish2, mu_coin::endpoint {}, true);
    client2.processor.process_message (publish3, mu_coin::endpoint {}, true);
    ASSERT_EQ (0, client1.conflicts.roots.size ());
    ASSERT_EQ (0, client2.conflicts.roots.size ());
    client1.processor.process_message (publish2, mu_coin::endpoint {}, true);
    client1.processor.process_message (publish3, mu_coin::endpoint {}, true);
	client2.processor.process_message (publish1, mu_coin::endpoint {}, true);
    ASSERT_EQ (1, client1.conflicts.roots.size ());
    ASSERT_EQ (1, client2.conflicts.roots.size ());
    auto conflict (client2.conflicts.roots.find (system.genesis.hash ()));
