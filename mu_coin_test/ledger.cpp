...
TEST (ledger, process_send)
{
    ...
    ASSERT_FALSE (store.identifier_get (key1.address ^ send.hash (), hash6));
    mu_coin::block_hash hash5;
    ASSERT_FALSE (store.latest_get (key1.address, hash5));
    ...
    ASSERT_FALSE (store.latest_get (key1.address, hash3));
    ...
    ASSERT_FALSE (store.identifier_get (key2.address ^ receive.hash (), hash7));
    mu_coin::block_hash hash4;
    ASSERT_FALSE (store.latest_get (key2.address, hash4));
