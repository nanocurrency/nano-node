...
std::shared_ptr<request_type> celerix::distributed_work::peer_request::get_prepared_json_request (std::string const & request_string_a) const
{
...
celerix::distributed_work::distributed_work (celerix::node & node_a, celerix::work_request const & request_a, std::chrono::seconds const & backoff_a) :
	node (node_a),
	...
	elapsed (celerix::timer_state::started, "distributed work generation timer")
{
	debug_assert (!finished);
	debug_assert (status == work_generation_status::ongoing);
}

celerix::distributed_work::~distributed_work ()
{
	debug_assert (status != work_generation_status::ongoing);
	if (auto node_l = node_w.lock ())
	{
		if (!node_l->stopped && node_l->websocket.server && node_l->websocket.server->any_subscriber (celerix::websocket::topic::work))
		{
...
void celerix::distributed_work::start ()
{
	...
	for (auto const & peer : need_resolve)
	{
		...
		if (!ec)
		{
			do_request (celerix::tcp_endpoint (parsed_address, peer.second));
		}
		else
		{
			...
			node.network.resolver.async_resolve (boost::asio::ip::tcp::resolver::query (peer.first, std::to_string (peer.second)), [peer, this_l, &extra = resolved_extra] (boost::system::error_code const & ec, boost::asio::ip::tcp::resolver::iterator i_a) {
				if (!ec)
				{
					this_l->do_request (celerix::tcp_endpoint (i_a->endpoint ().address (), i_a->endpoint ().port ()));
					++i_a;
					for (auto & i : boost::make_iterator_range (i_a, {}))
					{
						++extra;
						this_l->do_request (celerix::tcp_endpoint (i.endpoint ().address (), i.endpoint ().port ()));
					}
				}
				else
				{
					this_l->node.logger.error (celerix::log::type::distributed_work, "Error resolving work peer: {}:{} ({})", peer.first, peer.second, ec.message ());
...
void celerix::distributed_work::start_local ()
{
	auto this_l (shared_from_this ());
...
void celerix::distributed_work::do_request (celerix::tcp_endpoint const & endpoint_a)
{
	auto this_l (shared_from_this ());
	...
	[this_l, connection] (boost::system::error_code const & ec) {
		if (!ec && !this_l->stopped)
		{
			std::string request_string;
			{
				...
				rpc_request.put ("difficulty", celerix::to_string_hex (this_l->request.difficulty));
			...
			[this_l, connection, peer_request] (boost::system::error_code const & ec, std::size_t size_a) {
				if (!ec && !this_l->stopped)
				{
					...
					boost::asio::bind_executor (this_l->strand, [this_l, connection] (boost::system::error_code const & ec, std::size_t size_a) {
						if (!ec && !this_l->stopped)
						{
							...
							{
								this_l->node.logger.error (celerix::log::type::distributed_work, "Work peer responded with an error {}:{} ({})",
								celerix::util::to_str (connection->endpoint.address ()),
				...
				{
					this_l->node.logger.error (celerix::log::type::distributed_work, "Unable to write to work peer {}:{} ({})",
					celerix::util::to_str (connection->endpoint.address ()),
					connection->endpoint.port (),
					ec.message ());
		...
		{
			this_l->node.logger.error (celerix::log::type::distributed_work, "Unable to connect to work peer {}:{} ({})",
			celerix::util::to_str (connection->endpoint.address ()),
			connection->endpoint.port (),
			ec.message ());
...
void celerix::distributed_work::do_cancel (celerix::tcp_endpoint const & endpoint_a)
{
	auto this_l (shared_from_this ());
	...
	[this_l, cancelling_l] (boost::system::error_code const & ec) {
		if (!ec)
		{
			...
			[this_l, peer_cancel, cancelling_l] (boost::system::error_code const & ec, std::size_t bytes_transferred) {
				...
				{
					this_l->node.logger.error (celerix::log::type::distributed_work, "Unable to send work cancel to work peer {}:{} ({})",
					celerix::util::to_str (cancelling_l->endpoint.address ()),
...
void celerix::distributed_work::success (std::string const & body_a, celerix::tcp_endpoint const & endpoint_a)
{
	bool error = true;
	try
	{
		...
		uint64_t work;
		if (!celerix::from_string_hex (work_text, work))
		{
			if (celerix::dev::network_params.work.difficulty (request.version, request.root, work) >= request.difficulty)
			{
				error = false;
			...
			{
				node.logger.error (celerix::log::type::distributed_work, "Incorrect work response from {}:{} for root {} with difficulty {}: {}",
				celerix::util::to_str (endpoint_a.address ()),
				endpoint_a.port (),
				request.root.to_string (),
				celerix::to_string_hex (request.difficulty),
				work_text);
			}
		}
		else
		{
			node.logger.error (celerix::log::type::distributed_work, "Work response from {}:{} wasn't a number: {}",
			celerix::util::to_str (endpoint_a.address ()),
			endpoint_a.port (),
			work_text);
		}
	...
	{
		node.logger.error (celerix::log::type::distributed_work, "Work response from {}:{} wasn't parsable: {}",
		celerix::util::to_str (endpoint_a.address ()),
		endpoint_a.port (),
		body_a);
	}
...
void celerix::distributed_work::stop_once (bool const local_stop_a)
{
				...
				boost::asio::post (strand, boost::asio::bind_executor (strand, [this_l, connection_l] {
					...
					if (connection_l->socket.is_open ())
					{
						...
						if (!ec)
						{
							...
							if (ec)
							{
								this_l->node.logger.error (celerix::log::type::distributed_work, "Error closing socket with work peer: {}:{} ({})",
								celerix::util::to_str (connection_l->endpoint.address ()),
						...
						{
							this_l->node.logger.error (celerix::log::type::distributed_work, "Error cancelling operation with work peer: {}:{} ({})",
							celerix::util::to_str (connection_l->endpoint.address ()),
							connection_l->endpoint.port (),
							ec.message ());
						}
...
void celerix::distributed_work::set_once (uint64_t const work_a, std::string const & source_a)
{
	if (!finished.exchange (true))
	{
		elapsed.stop ();

		node.logger.info (celerix::log::type::distributed_work, "Work generation for {}, with a threshold difficulty of {} (multiplier {}x) complete: {} ms",
		request.root.to_string (),
		celerix::to_string_hex (request.difficulty),
		celerix::to_string (celerix::difficulty::to_multiplier (request.difficulty, node.default_difficulty (request.version)), 2),
		elapsed.value ().count ());
...
void celerix::distributed_work::cancel ()
{
	if (!finished.exchange (true))
	{
		elapsed.stop ();

		node.logger.info (celerix::log::type::distributed_work, "Work generation for {} was cancelled after {} ms",
		request.root.to_string (),
		elapsed.value ().count ());
...
void celerix::distributed_work::failure ()
{
...
void celerix::distributed_work::handle_failure ()
{
	if (!finished)
	{
		...
		{
			node.logger.info (celerix::log::type::distributed_work, "Work peer(s) failed to generate work for root {}, retrying... (backoff: {}s)",
			request.root.to_string (),
			backoff.count ());

			status = work_generation_status::failure_peers;

			std::weak_ptr<celerix::node> node_weak (node.shared ());
...
void celerix::distributed_work::add_bad_peer (celerix::tcp_endpoint const & endpoint_a)
{
	celerix::lock_guard<celerix::mutex> guard{ mutex };
