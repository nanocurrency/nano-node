...
inline celerix::build_flags operator| (celerix::build_flags a, celerix::build_flags b)
{
	return static_cast<celerix::build_flags> (static_cast<uint8_t> (a) | static_cast<uint8_t> (b));
}
inline uint8_t operator| (uint8_t a, celerix::build_flags b)
{
...
public:
	...
	abstract_builder & sign (celerix::raw_key const & private_key, celerix::public_key const & public_key);
...
protected:
	...
	uint8_t base_fields = static_cast<uint8_t> (celerix::build_flags::work_present | celerix::build_flags::signature_present);
};

/** Builder for state blocks */
class state_block_builder : public abstract_builder<celerix::state_block, state_block_builder>
{
public:
	/** Creates a state block builder by calling make_block() */
	state_block_builder ();
	/** Initialize from an existing block */
	state_block_builder & from (celerix::state_block const & block);
	...
	/** Set link */
	state_block_builder & link (celerix::link const & link);
	/** Set link from hex representation */
	state_block_builder & link_hex (std::string const & link_hex);
	/** Set link from an xrb_ or celerix_ address */
	state_block_builder & link_address (std::string const & link_address);
	/** Provides validation for build() */
	void validate ();

private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (celerix::build_flags::account_present | celerix::build_flags::balance_present | celerix::build_flags::link_present | celerix::build_flags::previous_present | celerix::build_flags::representative_present);
};

/** Builder for open blocks */
class open_block_builder : public abstract_builder<celerix::open_block, open_block_builder>
{
public:
	...
	open_block_builder & source (celerix::block_hash source);
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (celerix::build_flags::account_present | celerix::build_flags::representative_present | celerix::build_flags::link_present);
};

/** Builder for change blocks */
class change_block_builder : public abstract_builder<celerix::change_block, change_block_builder>
{
public:
	...
	/** Set representative */
	change_block_builder & representative (celerix::account account);
	/** Set representative from hex representation of public key */
	...
	/** Set previous block hash */
	change_block_builder & previous (celerix::block_hash previous);
	/** Set previous block hash from hex representation */
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (celerix::build_flags::previous_present | celerix::build_flags::representative_present);
};

/** Builder for send blocks */
class send_block_builder : public abstract_builder<celerix::send_block, send_block_builder>
{
public:
	/** Creates a send block builder by calling make_block() */
	send_block_builder ();
	/** Initialize from an existing block */
	send_block_builder & from (celerix::send_block const & block);
	/** Creates a new block with fields, signature and work set to sentinel values. All fields must be set or zeroed for build() to succeed. */
	send_block_builder & make_block ();
	/** Sets all hashables, signature and work to zero. */
	send_block_builder & zero ();
	/** Set destination */
	send_block_builder & destination (celerix::account account);
	/** Set destination from hex representation of public key */
	send_block_builder & destination_hex (std::string account_hex);
	/** Set destination from an xrb_ or celerix_ address */
	send_block_builder & destination_address (std::string account_address);
	/** Set previous block hash */
	send_block_builder & previous (celerix::block_hash previous);
	/** Set previous block hash from hex representation */
	send_block_builder & previous_hex (std::string previous_hex);
	/** Set balance */
	send_block_builder & balance (celerix::amount balance);
	/** Set balance from decimal string */
...
class receive_block_builder : public abstract_builder<celerix::receive_block, receive_block_builder>
{
public:
	...
	/** Set previous block hash */
	receive_block_builder & previous (celerix::block_hash previous);
	/** Set previous block hash from hex representation */
	receive_block_builder & previous_hex (std::string previous_hex);
	/** Set source block hash */
	receive_block_builder & source (celerix::block_hash source);
	/** Set source block hash from hex representation */
...
public:
	...
	celerix::state_block_builder & state ()
	{
	...
	celerix::open_block_builder & open ()
	{
	...
	celerix::change_block_builder & change ()
	{
	...
	celerix::send_block_builder & send ()
	{
	...
	celerix::receive_block_builder & receive ()
	{
...
private:
	...
	celerix::open_block_builder open_builder;
	celerix::change_block_builder change_builder;
	celerix::send_block_builder send_builder;
	celerix::receive_block_builder receive_builder;
};
}
