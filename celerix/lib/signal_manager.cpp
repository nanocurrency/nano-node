...
celerix::signal_manager::signal_manager () :
	work (boost::asio::make_work_guard (ioc))
{
	thread = std::thread ([&ioc = ioc] () {
		celerix::thread_role::set (celerix::thread_role::name::signal_manager);
		ioc.run ();
	});
}
...
celerix::signal_manager::signal_descriptor::signal_descriptor (std::shared_ptr<boost::asio::signal_set> sigset_a, signal_manager & sigman_a, std::function<void (int)> handler_func_a, bool repeat_a) :
	sigset (sigset_a), sigman (sigman_a), handler_func (handler_func_a), repeat (repeat_a)
{
}

void celerix::signal_manager::register_signal_handler (int signum, std::function<void (int)> handler, bool repeat)
{
	...
	sigset->async_wait ([descriptor] (boost::system::error_code const & error, int signum) {
		celerix::signal_manager::base_handler (descriptor, error, signum);
	});

	logger.debug (celerix::log::type::signal_manager, "Registered signal handler for signal: {}", to_signal_name (signum));
}

void celerix::signal_manager::base_handler (celerix::signal_manager::signal_descriptor descriptor, boost::system::error_code const & ec, int signum)
{
	auto & logger = descriptor.sigman.logger;

	if (!ec)
	{
		logger.debug (celerix::log::type::signal_manager, "Signal received: {}", to_signal_name (signum));
		...
		{
			logger.debug (celerix::log::type::signal_manager, "Signal handler {} will not repeat", to_signal_name (signum));
	...
	{
		logger.debug (celerix::log::type::signal_manager, "Signal error: {} ({})", ec.message (), to_signal_name (signum));
	}
}

std::string celerix::to_signal_name (int signum)
{
	switch (signum)
	{
		case SIGINT:
