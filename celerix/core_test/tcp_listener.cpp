...
TEST (tcp_listener, max_connections)
{
	...
	auto client1 = std::make_shared<celerix::transport::tcp_socket> (*node);
	client1->async_connect (node->network.endpoint (), connect_handler);

	auto client2 = std::make_shared<celerix::transport::tcp_socket> (*node);
	client2->async_connect (node->network.endpoint (), connect_handler);

	auto client3 = std::make_shared<celerix::transport::tcp_socket> (*node);
	client3->async_connect (node->network.endpoint (), connect_handler);

	ASSERT_TIMELY_EQ (5s, node->stats.count (celerix::stat::type::tcp_listener, celerix::stat::detail::accept_success), 2);
	ASSERT_ALWAYS_EQ (1s, node->stats.count (celerix::stat::type::tcp_listener, celerix::stat::detail::accept_success), 2);
	...
	auto client4 = std::make_shared<celerix::transport::tcp_socket> (*node);
	client4->async_connect (node->network.endpoint (), connect_handler);

	auto client5 = std::make_shared<celerix::transport::tcp_socket> (*node);
	client5->async_connect (node->network.endpoint (), connect_handler);

	ASSERT_TIMELY_EQ (5s, node->stats.count (celerix::stat::type::tcp_listener, celerix::stat::detail::accept_success), 3);
	ASSERT_ALWAYS_EQ (1s, node->stats.count (celerix::stat::type::tcp_listener, celerix::stat::detail::accept_success), 3);
	...
	auto client6 = std::make_shared<celerix::transport::tcp_socket> (*node);
	client6->async_connect (node->network.endpoint (), connect_handler);

	auto client7 = std::make_shared<celerix::transport::tcp_socket> (*node);
	client7->async_connect (node->network.endpoint (), connect_handler);

	auto client8 = std::make_shared<celerix::transport::tcp_socket> (*node);
	client8->async_connect (node->network.endpoint (), connect_handler);

	ASSERT_TIMELY_EQ (5s, node->stats.count (celerix::stat::type::tcp_listener, celerix::stat::detail::accept_success), 5);
	ASSERT_ALWAYS_EQ (1s, node->stats.count (celerix::stat::type::tcp_listener, celerix::stat::detail::accept_success), 5);
...
TEST (tcp_listener, max_connections_per_ip)
{
	...
	std::vector<std::shared_ptr<celerix::transport::tcp_socket>> client_list;
	...
	ASSERT_TIMELY_EQ (5s, node->stats.count (celerix::stat::type::tcp_listener, celerix::stat::detail::accept_success), max_ip_connections);
	ASSERT_TIMELY_EQ (5s, node->stats.count (celerix::stat::type::tcp_listener_rejected, celerix::stat::detail::max_per_ip), 1);
...
TEST (tcp_listener, max_connections_per_subnetwork)
{
	...
	ASSERT_TIMELY_EQ (5s, node->stats.count (celerix::stat::type::tcp_listener, celerix::stat::detail::accept_success), max_subnetwork_connections);
	ASSERT_TIMELY_EQ (5s, node->stats.count (celerix::stat::type::tcp_listener_rejected, celerix::stat::detail::max_per_subnetwork), 1);
...
TEST (tcp_listener, max_peers_per_ip)
{
	...
	ASSERT_TIMELY_EQ (5s, node->stats.count (celerix::stat::type::tcp_listener, celerix::stat::detail::accept_success), max_ip_connections + 1);
	ASSERT_TIMELY_EQ (5s, node->stats.count (celerix::stat::type::tcp_listener_rejected, celerix::stat::detail::max_per_ip), 0);
	ASSERT_TIMELY_EQ (5s, connection_attempts, max_ip_connections + 1);
}
...
TEST (tcp_listener, tcp_node_id_handshake)
{
	...
	auto socket (std::make_shared<celerix::transport::tcp_socket> (*system.nodes[0]));
	auto bootstrap_endpoint (system.nodes[0]->tcp_listener.endpoint ());
	auto cookie (system.nodes[0]->network.syn_cookies.assign (celerix::transport::map_tcp_to_endpoint (bootstrap_endpoint)));
	ASSERT_TRUE (cookie);
	celerix::node_id_handshake::query_payload query{ *cookie };
	celerix::node_id_handshake node_id_handshake{ celerix::dev::network_params.network, query };
	...
	celerix::node_id_handshake::response_payload response_zero{ 0 };
	celerix::node_id_handshake node_id_handshake_response{ celerix::dev::network_params.network, std::nullopt, response_zero };
...
// PR in which it got disabled: https://github.com/celerixcurrency/celerix-node/pull/3611
// Issue for investigating it: https://github.com/celerixcurrency/celerix-node/issues/3615
TEST (tcp_listener, DISABLED_tcp_listener_timeout_empty)
{
	celerix::test::system system (1);
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<celerix::transport::tcp_socket> (*node0));
...
TEST (tcp_listener, tcp_listener_timeout_node_id_handshake)
{
	...
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<celerix::transport::tcp_socket> (*node0));
	auto cookie (node0->network.syn_cookies.assign (celerix::transport::map_tcp_to_endpoint (node0->tcp_listener.endpoint ())));
	ASSERT_TRUE (cookie);
	celerix::node_id_handshake::query_payload query{ *cookie };
	celerix::node_id_handshake node_id_handshake{ celerix::dev::network_params.network, query };
	auto channel = std::make_shared<celerix::transport::tcp_channel> (*node0, socket);
	socket->async_connect (node0->tcp_listener.endpoint (), [&node_id_handshake, channel] (boost::system::error_code const & ec) {
		ASSERT_FALSE (ec);
		channel->send (node_id_handshake, celerix::transport::traffic_type::test, [] (boost::system::error_code const & ec, size_t size_a) {
			ASSERT_FALSE (ec);
		});
	});
	ASSERT_TIMELY (5s, node0->stats.count (celerix::stat::type::tcp_server, celerix::stat::detail::node_id_handshake) != 0);
