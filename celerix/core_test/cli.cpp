...
TEST (cli, key_create)
{
	...
	celerix::raw_key private_key;
	private_key.decode_hex (private_key_str);

	auto public_key = celerix::pub_key (private_key);
...
{
	std::vector<celerix::config_key_value_pair> key_value_pairs;
	auto config_overrides = celerix::config_overrides (key_value_pairs);
	ASSERT_TRUE (config_overrides.empty ());
	key_value_pairs.push_back ({ "key", "value" });
	config_overrides = celerix::config_overrides (key_value_pairs);
