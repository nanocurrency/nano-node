#include <celerix/core_test/fakes/websocket_client.hpp>
...
TEST (websocket, subscription_edge)
{
	...
	ASSERT_EQ (0, node1->websocket.server->subscriber_count (celerix::websocket::topic::confirmation));
	...
	auto task = ([config, &node1] () {
		...
		client.await_ack ();
		EXPECT_EQ (1, node1->websocket.server->subscriber_count (celerix::websocket::topic::confirmation));
		client.send_message (R"json({"action": "subscribe", "topic": "confirmation", "ack": true})json");
		client.await_ack ();
		EXPECT_EQ (1, node1->websocket.server->subscriber_count (celerix::websocket::topic::confirmation));
		client.send_message (R"json({"action": "unsubscribe", "topic": "confirmation", "ack": true})json");
		client.await_ack ();
		EXPECT_EQ (0, node1->websocket.server->subscriber_count (celerix::websocket::topic::confirmation));
		client.send_message (R"json({"action": "unsubscribe", "topic": "confirmation", "ack": true})json");
		client.await_ack ();
		EXPECT_EQ (0, node1->websocket.server->subscriber_count (celerix::websocket::topic::confirmation));
	});
...
TEST (websocket, started_election)
{
	...
	auto task = ([&ack_ready, config, &node1] () {
		...
		client.await_ack ();
		ack_ready = true;
		EXPECT_EQ (1, node1->websocket.server->subscriber_count (celerix::websocket::topic::started_election));
...
TEST (websocket, stopped_election)
{
	...
	auto task = ([&ack_ready, config, &node1] () {
		...
		client.await_ack ();
		ack_ready = true;
		EXPECT_EQ (1, node1->websocket.server->subscriber_count (celerix::websocket::topic::stopped_election));
		return client.get_response ();
	});
	auto future = std::async (std::launch::async, task);
...
TEST (websocket, confirmation_options_votes)
{
	...
	try
	{
		...
		for (auto & vote : votes_l.get ())
		{
			...
			ASSERT_EQ (celerix::dev::genesis_key.pub.to_account (), representative);
			...
			ASSERT_EQ (node1->latest (celerix::dev::genesis_key.pub).to_string (), hash);
			std::string weight (vote.second.get<std::string> ("weight"));
			ASSERT_EQ (node1->balance (celerix::dev::genesis_key.pub).convert_to<std::string> (), weight);
		}
	}
	catch (std::runtime_error const & ex)
	{
...
TEST (websocket, confirmation_options_update)
{
	...
	auto task = ([&added, &deleted, config, &node1] () {
		...
		std::string add_message = boost::str (boost::format (R"json({"action": "update", "topic": "confirmation", "ack": "true", "options": {"accounts_add": ["%1%"]}})json") % celerix::dev::genesis_key.pub.to_account ());
		...
		std::string delete_message = boost::str (boost::format (R"json({"action": "update", "topic": "confirmation", "ack": "true", "options": {"accounts_del": ["%1%"]}})json") % celerix::dev::genesis_key.pub.to_account ());
	...
	celerix::keypair key;
	celerix::state_block_builder builder;
	auto previous (node1->latest (celerix::dev::genesis_key.pub));
	auto send = builder
				.account (celerix::dev::genesis_key.pub)
				.previous (previous)
				.representative (celerix::dev::genesis_key.pub)
...
TEST (websocket, vote)
{
	...
	auto task = ([&ack_ready, config, &node1] () {
		...
		client.await_ack ();
		ack_ready = true;
		EXPECT_EQ (1, node1->websocket.server->subscriber_count (celerix::websocket::topic::vote));
		return client.get_response ();
	});
	auto future = std::async (std::launch::async, task);
	...
	auto send = builder
				...
				.previous (previous)
				.representative (celerix::dev::genesis_key.pub)
				.balance (celerix::dev::constants.genesis_amount - (node1->online_reps.delta () + 1))
				.link (key.pub)
				.sign (celerix::dev::genesis_key.prv, celerix::dev::genesis_key.pub)
				.work (*system.work.generate (previous))
				.build ();
...
TEST (websocket, vote_options_type)
{
	...
	auto vote = celerix::test::make_vote (celerix::dev::genesis_key, { celerix::dev::genesis }, 0, 0);
	celerix::websocket::message_builder builder;
	auto msg (builder.vote_received (vote, celerix::vote_code::replay));
...
TEST (websocket, vote_options_representatives)
{
	...
	auto task1 = ([&ack_ready, config, &node1] () {
		fake_websocket_client client (node1->websocket.server->listening_port ());
		std::string message = boost::str (boost::format (R"json({"action": "subscribe", "topic": "vote", "ack": "true", "options": {"representatives": ["%1%"]}})json") % celerix::dev::genesis_key.pub.to_account ());
...
TEST (websocket, work)
{
	...
	ASSERT_EQ (0, node1->websocket.server->subscriber_count (celerix::websocket::topic::work));
	...
	auto task = ([&ack_ready, config, &node1] () {
		...
		client.await_ack ();
		ack_ready = true;
		EXPECT_EQ (1, node1->websocket.server->subscriber_count (celerix::websocket::topic::work));
		return client.get_response ();
	});
	auto future = std::async (std::launch::async, task);
	...
	ASSERT_TIMELY (5s, ack_ready);
	ASSERT_EQ (1, node1->websocket.server->subscriber_count (celerix::websocket::topic::work));
	...
	ASSERT_EQ (request.get<std::string> ("version"), celerix::to_string (celerix::work_version::work_1));
	ASSERT_EQ (request.get<std::string> ("hash"), hash.to_string ());
	ASSERT_EQ (request.get<std::string> ("difficulty"), celerix::to_string_hex (node1->default_difficulty (celerix::work_version::work_1)));
	ASSERT_EQ (request.get<double> ("multiplier"), 1.0);

	ASSERT_EQ (1, contents.count ("result"));
	auto & result = contents.get_child ("result");
	uint64_t result_difficulty;
	celerix::from_string_hex (result.get<std::string> ("difficulty"), result_difficulty);
	ASSERT_GE (result_difficulty, node1->default_difficulty (celerix::work_version::work_1));
	ASSERT_NEAR (result.get<double> ("multiplier"), celerix::difficulty::to_multiplier (result_difficulty, node1->default_difficulty (celerix::work_version::work_1)), 1e-6);
	ASSERT_EQ (result.get<std::string> ("work"), celerix::to_string_hex (work.value ()));
...
TEST (websocket, telemetry)
{
	...
	auto task = ([config = node1->config, &node1, &done] () {
		...
		EXPECT_EQ (1, node1->websocket.server->subscriber_count (celerix::websocket::topic::telemetry));
		return client.get_response ();
	});
	...
	auto & contents = event.get_child ("message");
	celerix::jsonconfig telemetry_contents (contents);
	celerix::telemetry_data telemetry_data;
	telemetry_data.deserialize_json (telemetry_contents, false);

	ASSERT_TRUE (celerix::test::compare_telemetry (telemetry_data, *node2));
	...
	EXPECT_EQ (0, node2->websocket.server->subscriber_count (celerix::websocket::topic::telemetry));
}
...
TEST (websocket, new_unconfirmed_block)
{
	...
	auto task = ([&ack_ready, config, node1] () {
		...
		client.await_ack ();
		ack_ready = true;
		EXPECT_EQ (1, node1->websocket.server->subscriber_count (celerix::websocket::topic::new_unconfirmed_block));
		return client.get_response ();
	});
	auto future = std::async (std::launch::async, task);
	...
	ASSERT_EQ (celerix::block_status::progress, node1->process_local (send1).value ());
