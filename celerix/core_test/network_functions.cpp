...
{
	// 0 port test
	ASSERT_TRUE (celerix::transport::reserved_address (celerix::endpoint (boost::asio::ip::make_address_v6 ("2001::"), 0)));
	// Valid address test
	ASSERT_FALSE (celerix::transport::reserved_address (celerix::endpoint (boost::asio::ip::make_address_v6 ("2001::"), 1)));
	celerix::endpoint loopback (boost::asio::ip::make_address_v6 ("::1"), 1);
	ASSERT_FALSE (celerix::transport::reserved_address (loopback));
	celerix::endpoint private_network_peer (boost::asio::ip::make_address_v6 ("::ffff:10.0.0.0"), 1);
	ASSERT_TRUE (celerix::transport::reserved_address (private_network_peer, false));
	ASSERT_FALSE (celerix::transport::reserved_address (private_network_peer, true));
}
...
TEST (network_functions, ipv6_bind_subnetwork)
{
	...
	auto address1_subnet (celerix::transport::ipv4_address_or_ipv6_subnet (address1));
	...
	auto address2_subnet (celerix::transport::ipv4_address_or_ipv6_subnet (address2));
...
TEST (network_functions, network_range_ipv6)
{
	...
	auto address3_subnet (celerix::transport::map_address_to_subnetwork (address3));
...
TEST (network_functions, network_range_ipv4)
{
	...
	auto address2_subnet (celerix::transport::map_address_to_subnetwork (address2));
...
TEST (network_functions, ipv4_address_or_ipv6_subnet)
{
	...
	ASSERT_EQ (celerix::transport::ipv4_address_or_ipv6_subnet (addr1), addr2);
	...
	ASSERT_EQ (celerix::transport::ipv4_address_or_ipv6_subnet (addr3), addr4);
	...
	ASSERT_NE (celerix::transport::ipv4_address_or_ipv6_subnet (addr3), celerix::transport::ipv4_address_or_ipv6_subnet (addr5));
}
...
TEST (network_functions, is_same_ip)
{
	...
	ASSERT_TRUE (celerix::transport::is_same_ip (ipv4_addr1, ipv4_addr1));

	// IPv4 and its IPv6 mapped form
	boost::asio::ip::address ipv6_mapped_ipv4 = boost::asio::ip::address::from_string ("::ffff:192.168.1.1");
	ASSERT_TRUE (celerix::transport::is_same_ip (ipv4_addr1, ipv6_mapped_ipv4));
}
...
TEST (network_functions, is_same_ipv6)
{
	...
	ASSERT_TRUE (celerix::transport::is_same_ip (ipv6_addr1, ipv6_addr2));
	...
	ASSERT_FALSE (celerix::transport::is_same_ip (ipv6_addr1, ipv6_addr3));
}
...
TEST (network_functions, is_different_ip_family)
{
	...
	ASSERT_FALSE (celerix::transport::is_same_ip (addr1, addr2));
}
...
TEST (network_functions, is_same_ip_v4_mapped)
{
	...
	ASSERT_TRUE (celerix::transport::is_same_ip (addr1, addr2));

	boost::asio::ip::address addr3 = boost::asio::ip::address::from_string ("10.0.0.1");
	ASSERT_FALSE (celerix::transport::is_same_ip (addr1, addr3));
}
...
TEST (network_functions, map_ipv4_address_to_subnetwork)
{
	...
	auto subnetwork = celerix::transport::map_address_to_subnetwork (addr);
